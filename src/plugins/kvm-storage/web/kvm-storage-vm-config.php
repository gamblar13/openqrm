	...
	if ($OPENQRM_USER->role == "administrator") {
		...
		switch (htmlobject_request('action')) {
			case 'update_cpus':
					...
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/kvm-storage/bin/openqrm-kvm-storage-vm update_vm_cpus -n $kvm_server_name -c $kvm_update_cpus -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
			...
			case 'update_ram':
					...
					$kvm_server = new resource();
					$kvm_server->get_instance_by_id($kvm_server_appliance->resources);
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/kvm-storage/bin/openqrm-kvm-storage-vm update_vm_ram -n $kvm_server_name -r $kvm_update_ram -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$kvm_server_resource_id = $kvm_server->id;
					$statfile="kvm-stat/".$kvm_server_resource_id.".".$kvm_server_name.".vm_config";
					if (file_exists($statfile)) {
			...
			case 'update_vnc':
					...
					$kvm_server = new resource();
					$kvm_server->get_instance_by_id($kvm_server_appliance->resources);
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/kvm-storage/bin/openqrm-kvm-storage-vm update_vm_vnc -n $kvm_server_name -v $kvm_update_vncpassword -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$kvm_server_resource_id = $kvm_server->id;
					$statfile="kvm-stat/".$kvm_server_resource_id.".".$kvm_server_name.".vm_config";
					if (file_exists($statfile)) {
			...
			case 'add_vm_net':
					...
					$kvm_server = new resource();
					$kvm_server->get_instance_by_id($kvm_server_appliance->resources);
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/kvm-storage/bin/openqrm-kvm-storage-vm add_vm_nic -n $kvm_server_name -x $kvm_nic_nr -m $kvm_new_nic -t $kvm_nic_model -z $kvm_vm_bridge -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$kvm_server_resource_id = $kvm_server->id;
					$statfile="kvm-stat/".$kvm_server_resource_id.".".$kvm_server_name.".vm_config";
					if (file_exists($statfile)) {
			...
			case 'remove_vm_net':
					...
					$kvm_server = new resource();
					$kvm_server->get_instance_by_id($kvm_server_appliance->resources);
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/kvm-storage/bin/openqrm-kvm-storage-vm remove_vm_nic -n $kvm_server_name -x $kvm_nic_nr -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$kvm_server_resource_id = $kvm_server->id;
					$statfile="kvm-stat/".$kvm_server_resource_id.".".$kvm_server_name.".vm_config";
					if (file_exists($statfile)) {
			...
			case 'update_boot_dev':
					...
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/kvm-storage/bin/openqrm-kvm-storage-vm setboot_by_name -n $kvm_server_name -b $kvm_vm_boot_dev $kvm_vm_boot_iso -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$kvm_server_resource_id = $kvm_server->id;
					$statfile="kvm-stat/".$kvm_server_resource_id.".".$kvm_server_name.".vm_config";
					if (file_exists($statfile)) {
...
function kvm_vm_config() {
	...
	$html->name = "net1";
	$html->id = 'p'.uniqid();
	$html->value = $store['OPENQRM_KVM_VM_MAC_1']." / bridge ".$store['OPENQRM_KVM_VM_BRIDGE_1'];
	...
	if (isset($store['OPENQRM_KVM_VM_MAC_2'])) {
		if (strlen($store['OPENQRM_KVM_VM_MAC_2'])) {
			...
			$html->name = "net2";
			$html->id = 'p'.uniqid();
			$html->value = $store['OPENQRM_KVM_VM_MAC_2']." / bridge ".$store['OPENQRM_KVM_VM_BRIDGE_2'];
	...
	if (isset($store['OPENQRM_KVM_VM_MAC_3'])) {
		if (strlen($store['OPENQRM_KVM_VM_MAC_3'])) {
			...
			$html->name = "net3";
			$html->id = 'p'.uniqid();
			$html->value = $store['OPENQRM_KVM_VM_MAC_3']." / bridge ".$store['OPENQRM_KVM_VM_BRIDGE_3'];
	...
	if (isset($store['OPENQRM_KVM_VM_MAC_4'])) {
		if (strlen($store['OPENQRM_KVM_VM_MAC_4'])) {
			...
			$html->name = "net4";
			$html->id = 'p'.uniqid();
			$html->value = $store['OPENQRM_KVM_VM_MAC_4']." / bridge ".$store['OPENQRM_KVM_VM_BRIDGE_4'];
	...
	if (isset($store['OPENQRM_KVM_VM_MAC_5'])) {
		if (strlen($store['OPENQRM_KVM_VM_MAC_5'])) {
			...
			$html->name = "net5";
			$html->id = 'p'.uniqid();
			$html->value = $store['OPENQRM_KVM_VM_MAC_5']." / bridge ".$store['OPENQRM_KVM_VM_BRIDGE_5'];
...
function kvm_vm_config_net() {
	...
	if ($nic_number < 6) {
		...
		$kvm_bridge_net1 = $bridge_store['OPENQRM_PLUGIN_KVM_BRIDGE_NET1'];
		$kvm_bridge_net2 = $bridge_store['OPENQRM_PLUGIN_KVM_BRIDGE_NET2'];
		$kvm_bridge_net3 = $bridge_store['OPENQRM_PLUGIN_KVM_BRIDGE_NET3'];
		$kvm_bridge_net4 = $bridge_store['OPENQRM_PLUGIN_KVM_BRIDGE_NET4'];
		$kvm_bridge_net5 = $bridge_store['OPENQRM_PLUGIN_KVM_BRIDGE_NET5'];
		$bridge_identifier_array = array();
		$bridge_identifier_array[] = array("value" => "1", "label" => "$kvm_bridge_net1 (bridge 1)");
		$bridge_identifier_array[] = array("value" => "2", "label" => "$kvm_bridge_net2 (bridge 2)");
		$bridge_identifier_array[] = array("value" => "3", "label" => "$kvm_bridge_net3 (bridge 3)");
		$bridge_identifier_array[] = array("value" => "4", "label" => "$kvm_bridge_net4 (bridge 4)");
		$bridge_identifier_array[] = array("value" => "5", "label" => "$kvm_bridge_net5 (bridge 5)");
		$bridge_select = htmlobject_select('kvm_vm_bridge', $bridge_identifier_array, 'Network-Bridge', array(0));
...
function kvm_vm_config_vnc() {
   ...
   // set template
	$t = new Template_PHPLIB();
	$t->debug = false;
	$t->setFile('tplfile', './tpl/' . 'kvm-storage-vm-config-vnc.tpl.php');
	$t->setVar(array(
