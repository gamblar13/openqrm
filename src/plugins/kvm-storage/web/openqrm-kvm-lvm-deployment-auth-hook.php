	...
	*/
	...
	function storage_auth_function($cmd, $appliance_id) {
        ...
        if ($vm_host_resource->id != $storage_resource->id) {
            ...
            $event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "openqrm-kvm-lvm-deployment-auth-hook.php", "Appliance $appliance_id image IS available on this kvm-storage host, $storage_resource->id equal $vm_host_resource->id .. $appliance_name/$appliance_ip", "", "", 0, 0, $appliance_id);
        }
		...
		switch($cmd) {
			case "start":
				...
				$event->log("storage_auth_function", $_SERVER['REQUEST_TIME'], 5, "openqrm-kvm-lvm-deployment-auth-hook.php", "Authenticating $image_name / $image_location_name to resource $resource_mac", "", "", 0, 0, $appliance_id);
