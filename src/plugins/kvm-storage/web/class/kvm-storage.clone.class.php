...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/kvm-storage-clone.tpl.php');
	...
	function duplicate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$command  = $this->openqrm->get('basedir').'/plugins/kvm-storage/bin/openqrm-kvm-storage clone';
			...
			$statfile = $this->openqrm->get('basedir').'/plugins/kvm-storage/web/storage/'.$this->resource->id.'.'.$this->volgroup.'.lv.stat';
			...
			if(isset($error)) {
				...
				$file = $this->openqrm->get('basedir').'/plugins/kvm-storage/web/storage/'.$this->resource->id.'.lvm.'.$name.'.sync_progress';
				if($this->file->exists($file)) {
					$this->file->remove($file);
				}
				$root_device_ident = $this->openqrm->get('basedir').'/plugins/kvm-storage/web/storage/'.$this->resource->id.'.'.$name.'.root_device';
