...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/kvm-storage-vm-migrate.tpl.php');
	...
	function migrate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$s_command     = $this->openqrm->get('basedir').'/plugins/kvm-storage/bin/openqrm-kvm-storage-vm start_as_incoming';
			$s_command    .= ' -n '.$this->vm;
			$s_command    .= ' -j '.$vm_resource->id;
			$s_command    .= ' -u '.$this->openqrm->admin()->name.' -p '.$this->openqrm->admin()->password;

			$statfile=$this->openqrm->get('basedir').'/plugins/kvm-storage/web/kvm-stat/'.$this->vm.'.vm_migrated_successfully';
			...
			$m_command     = $this->openqrm->get('basedir').'/plugins/kvm-storage/bin/openqrm-kvm-storage-vm migrate';
			$m_command    .= ' -n '.$this->vm;
