...
var $lang = array();
	...
	function init() {
		...
		$this->statfile = $this->openqrm->get('basedir').'/plugins/kvm-storage/web/storage/'.$resource->id.'.'.$this->volgroup.'.lv.stat';
	}
	...
	function action() {
		...
		$data = $this->edit();
		if($data !== false) {
			$t = $this->response->html->template($this->tpldir.'/kvm-storage-volgroup.tpl.php');
	...
	function edit() {
		if(strpos($this->deployment->type, 'kvm') !== false) {
			...
			// Volgroup info
			$lines = explode("\n", $this->file->get_contents($this->openqrm->get('basedir').'/plugins/kvm-storage/web/storage/'.$this->resource->id.'.vg.stat'));
					...
					foreach($lines as $line) {
						if($line !== '') {
							...
							if ($line[4] == "clone_in_progress") {
								...
								// progressbar
								$t->add(uniqid('b'), 'id');
								$t->add('/openqrm/base/plugins/kvm-storage/storage/'.$this->resource->id.'.lvm.'.$name.'.sync_progress', 'url');
