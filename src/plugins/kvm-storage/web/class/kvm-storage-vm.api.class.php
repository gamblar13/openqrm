...
class kvm_storage_vm_api
{
	...
	function filepicker() {
		...
		if ($iso_path !== '') {
		    ...
		    $command  = $this->controller->openqrm->get('basedir')."/plugins/kvm-storage/bin/openqrm-kvm-storage-vm iso";
		    $command .= ' -q '.$iso_path;
		    $command .= ' -u '.$this->admin->name.' -p '.$this->admin->password;

		    $file = 'kvm-stat/'.$this->resource->id.'.pick_iso_config';
