	...
	function stop() {
		...
		if( $vms !== '' ) {
			...
			foreach($vms as $key => $vm) {
				...
				$file = $this->openqrm->get('basedir').'/plugins/kvm-storage/web/kvm-stat/'.$resource->id.'.vm_list';
				$command  = $this->openqrm->get('basedir').'/plugins/kvm-storage/bin/openqrm-kvm-storage-vm stop -n '.$vm;
						...
						foreach ($enabled as $index => $name) {
							...
							if (file_exists($hook)) {
								if (file_exists($running)) {
									$event->log("console", $_SERVER['REQUEST_TIME'], 5, "kvm-storage-vm.console.class.php", 'Found plugin '.$name.' providing a remote console.', "", "", 0, 0, $resource->id);
									require_once($hook);
