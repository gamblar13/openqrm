...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/kvm-storage-resize.tpl.php');
	...
	function resize() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(!$form->get_errors()) {
				...
				$command  = $this->openqrm->get('basedir').'/plugins/kvm-storage/bin/openqrm-kvm-storage resize';
	...
	function set_max() {
		...
		$statfile = $this->openqrm->get('basedir').'/plugins/kvm-storage/web/storage/'.$this->resource->id.'.vg.stat';
		if ($this->file->exists($statfile)) {
			$lines = explode("\n", $this->file->get_contents($statfile));
			if(count($lines) >= 1) {
				foreach($lines as $line) {
