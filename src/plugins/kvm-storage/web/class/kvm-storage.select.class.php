...
var $lang = array();
	...
	function action() {
		$table = $this->select();
		$t = $this->response->html->template($this->tpldir.'/kvm-storage-select.tpl.php');
	...
	function select() {
		...
		if(count($storages) >= 1) {
			foreach($storages as $k => $v) {
				...
				if($deployment->storagetype === 'kvm-storage') {
