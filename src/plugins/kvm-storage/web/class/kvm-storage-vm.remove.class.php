...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/kvm-storage-vm-remove.tpl.php');
								...
								if($line !== '') {
									...
									if($vm === $line[1]) {
										...
										if (count($appliances_using_resource) > 0) {
											...
											$command  = $this->openqrm->get('basedir').'/plugins/kvm-storage/bin/openqrm-kvm-storage-vm delete -n '.$vm;
