...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/kvm-storage-vm-add.tpl.php');
	...
	function add() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(count($errors) > 0 || $form->get_errors()) {
				...
				$command  = $this->openqrm->get('basedir').'/plugins/kvm-storage/bin/openqrm-kvm-storage-vm create';
				$command .= ' -n '.$name;
				$command .= ' -m '.$mac;
	...
	function get_response() {
		...
		$file = $OPENQRM_SERVER_BASE_DIR.'/plugins/kvm-storage/web/kvm-stat/'.$this->resource->id.'.bridge_config';
		$data = openqrm_parse_conf($file);
		$bridges = array();
		foreach($data as $key => $bridge) {
			$key = str_replace('OPENQRM_PLUGIN_KVM_BRIDGE_NET', '', $key);
			$bridges[] = array($key, $bridge.' (bridge'.$key.')');
		}
