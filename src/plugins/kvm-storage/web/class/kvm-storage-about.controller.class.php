...
class kvm_storage_about_controller
{
/**
...
* @var string
*/
var $actions_name = 'kvm_storage_about_action';
/**
* message param
* @access public
* @var string
*/
var $message_param = 'kvm_storage_about_msg';
/**
* id for tabs
* @access public
* @var string
*/
var $prefix_tab = 'kvm_storage_about_tab';
/**
* identifier name
* @access public
* @var string
*/
var $identifier_name = 'kvm_storage_about_identifier';
/**
* path to templates
* @access public
* @var string
...
var $lang = array(
	'documentation' => array (
		...
		'label' => 'About KVM-Storage',
		'introduction_title' => 'Introduction',
		'introduction_content' => 'The "KVM-Storage" plugin manages KVM Virtual Machines and their belonging virtual disk.
					   As common in openQRM the Virtual Machines and their virtual disk volumes are managed separately.
					   Therefore the "KVM-Storage" plugin splits up into VM- and Volume-Management.
		...
		'requirements_list' => '<li>A resource for the KVM-Storage Host Appliance<br>(this can be a remote system integrated into openQRM e.g. via the "local-server" plugin or the openQRM server itself)</li>
		...
		'provides_list' => '<li>Virtualization types: "KVM-Storage Host" and "KVM-Storage VM"</li>
				   <li>Storage types: "KVM LVM Storage" and "KVM Blockfile Storage"</li>
				   <li>Deployment types: "LVM deployment for KVM" and "Blockfile deployment for KVM"</li>',
		...
		'migration_content' => 'Shared storage between the KVM-Storage Hosts for the location of the VM config files (/var/lib/kvm-storage/openqrm)
					and a shared LVM volume group between the KVM-Storage Hosts',
	...
	'bootservice' => array (
		...
		'boot_service_content' => 'The KVM-Storage Plugin provides an openQRM Boot-Service.
			This "KVM-Storage Boot-Service" is automatically downloaded and executed by the openQRM-Client on all integrated Systems.
			The Boot-Service is located at:<br>
			<br>
				<i><b>/usr/share/openqrm/plugins/kvm-storage/web/boot-service-kvm-storage.tgz</b></i>
			<br>
			<br>
			The "KVM-Storage Boot-Service" contains the Client files of the KVM-Storage Plugin.<br>
			Also a configuration file for the KVM-Storage Hosts is included in this Boot-Service.<br>
			<br>
			...
			<br>
			<br>
			In case the openmQRM Server itself is used as the KVM-Storage Host please edit:<br>
			<br>
	...
	'storage' => array (
		'tab' => 'About KVM-Storage',
		'label' => 'About Storage in KVM-Storage',
		'storage_mgmt_title' => 'KVM Storage Management',
		'storage_mgmt_list' => '<li>Create a new storage from type "KVM LVM Storage" or "KVM Blockfile Storage"</li>
	...
	'vms' => array (
		'tab' => 'About KVM-Storage',
		'label' => 'About Virtual Machines in KVM-Storage',
		'vm_mgmt_title' => 'KVM-Storage VM Management',
		'vm_mgmt_list' => '<li>Create a new appliance and set its resource type to "KVM-Storage Host"</li>
				   <li>Create and manage KVM virtual machines via the KVM-Storage VM Manager</li>',
	),
	'usage' => array (
		'tab' => 'About KVM-Storage',
		'label' => 'KVM-Storage Use-Cases',
	),
	...
	function __construct($openqrm, $response) {
		...
		$this->lang     = $this->user->translate($this->lang, $this->rootdir."/plugins/kvm-storage/lang", 'kvm-storage-about.ini');


		$this->tpldir   = $this->rootdir.'/plugins/kvm-storage/tpl';
	}
	...
	function documentation( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage-about.documentation.class.php');
			$controller = new kvm_storage_about_documentation($this->openqrm, $this->response);
	...
	function bootservice( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage-about.bootservice.class.php');
			$controller = new kvm_storage_about_bootservice($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
	...
	function storage( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage-about.storage.class.php');
			$controller = new kvm_storage_about_storage($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
	...
	function vms( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage-about.vms.class.php');
			$controller = new kvm_storage_about_vms($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
	...
	function usage( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage-about.usage.class.php');
			$controller = new kvm_storage_about_usage($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
