...
var $lang = array(
	'select' => array (
		'tab' => 'Select KVM-Storage',
		'label' => 'Select KVM-Storage',
		'action_edit' => 'edit',
		'table_name' => 'Name',
		'table_id' => 'Id',
		...
		'error_no_storage' => '<b>No storage configured yet!</b><br><br>Please create a KVM-Storage first!',
		'new_storage' => 'New Storage',
		'please_wait' => 'Loading Volume groups. Please wait ..',
	), 
	'edit' => array (
		'tab' => 'Select Volume group',
		'label' => 'Select Volume group on KVM-Storage %s',
		'lang_id' => 'ID',
		'lang_name' => 'Name',
		...
		'table_vsize' => 'Vsize',
		'table_vfree' => 'VFree',
		'error_no_kvm' => 'Storage %s is not of type kvm-storage',
		'please_wait' => 'Loading Volume group. Please wait ..',
	),
	'volgroup' => array (
		'tab' => 'Edit Volume group',
		'label' => 'Edit Volume group %s on KVM-Storage %s',
		'lang_id' => 'ID',
		'lang_name' => 'Name',
		'lang_deployment' => 'Deployment',
		'lang_resource' => 'Resource',
	...
	function __construct($openqrm, $response) {
		...
		$this->lang     = $this->user->translate($this->lang, $this->rootdir."/plugins/kvm-storage/lang", 'kvm-storage.ini');
	...
	//--------------------------------------------
	function api() {
		require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage.api.class.php');
		$controller = new kvm_storage_api($this);
		$controller->action();
	}
	...
	function select( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage.select.class.php');
			$controller = new kvm_storage_select($this->openqrm, $this->response);
	...
	function edit( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage.edit.class.php');
			$controller                  = new kvm_storage_edit($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
	...
	function volgroup( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage.volgroup.class.php');
			$controller                  = new kvm_storage_volgroup($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function add( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage.add.class.php');
			$controller                = new kvm_storage_add($this->openqrm, $this->response, $this);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function remove( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage.remove.class.php');
			$controller                  = new kvm_storage_remove($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function snap( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage.snap.class.php');
			$controller                  = new kvm_storage_snap($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function duplicate( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage.clone.class.php');
			$controller                  = new kvm_storage_clone($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function resize( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/kvm-storage/class/kvm-storage.resize.class.php');
			$controller                  = new kvm_storage_resize($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function reload($mode) {
		...
		// reload volume group
		if($mode === 'vg') {
			$file = $OPENQRM_SERVER_BASE_DIR.'/plugins/kvm-storage/web/storage/'.$resource->id.'.vg.stat';
			$command .= $OPENQRM_SERVER_BASE_DIR.'/plugins/kvm-storage/bin/openqrm-kvm-storage post_vg -t '.$deployment->type;
		}
		// reload logical volumes
		if($mode === 'lv') {
			$file = $OPENQRM_SERVER_BASE_DIR.'/plugins/kvm-storage/web/storage/'.$resource->id.'.'.$volgroup.'.lv.stat';
			$command .= $OPENQRM_SERVER_BASE_DIR.'/plugins/kvm-storage/bin/openqrm-kvm-storage post_lv';
