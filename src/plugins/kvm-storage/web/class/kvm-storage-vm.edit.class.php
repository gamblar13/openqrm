...
var $lang = array();
	...
	function action() {
		...
		$data = $this->edit();
		if($data !== false) {
			$t = $this->response->html->template($this->tpldir.'/kvm-storage-vm-edit.tpl.php');
	...
	function edit() {
		...
		$d['icon'] = '<img width="24" height="24" src="'.$this->openqrm->get('baseurl').'/plugins/kvm-storage/img/plugin.png">';
				...
				foreach($lines as $line) {
					if($line !== '') {
						...
						if($state !== '2') {						
							...
							// progressbar
							$t->add(uniqid('b'), 'id');
							$t->add($this->openqrm->get('baseurl').'/plugins/kvm-storage/kvm-stat/'.$name.'.vm_migration_progress', 'url');
