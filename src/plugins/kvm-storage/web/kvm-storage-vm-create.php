...
if(htmlobject_request('action') != '') {
    ...
    switch ($action) {
        case 'new':
            ...
            // final command
            $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/kvm-storage/bin/openqrm-kvm-storage-vm create -n $kvm_server_name -m $kvm_server_mac -r $kvm_server_ram -c $kvm_server_cpus -t $kvm_nic_model -z $kvm_vm_bridge -b $kvm_vm_boot_dev $kvm_vm_boot_iso -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
...
function kvm_server_create($kvm_server_id) {
	...
	$t->setVar(array(
		...
		'kvm_server_bridge_int' => $store[OPENQRM_PLUGIN_KVM_INTERNAL_BRIDGE],
		'kvm_server_bridge_ext' => $store[OPENQRM_PLUGIN_KVM_EXTERNAL_BRIDGE],
