...
var filepicker = {
	init : function() {
		...
		$.ajax({
			url: "{baseurl}/plugins/kvm-storage/api-vm.php?appliance_id={appliance_id}&{actions_name}=filepicker&path=/",
			dataType: "text",
	...
	browse : function(target) {
		document.getElementById('canvas').innerHTML = '<img src="../../img/loading.gif" style="margin-top:150px;">';
		$.ajax({
			url: "{baseurl}/plugins/kvm-storage/api-vm.php?appliance_id={appliance_id}&{actions_name}=filepicker&path="+target,
			dataType: "text",
			success: function(response) {
				document.getElementById('canvas').innerHTML = response;	
			}
