...
function create_clone_kvm_lvm_deployment($cloud_image_id, $image_clone_name, $disk_size) {
	...
	global $RESOURCE_INFO_TABLE;
	global $event;
	$event->log("create_clone_kvm_lvm_deployment", $_SERVER['REQUEST_TIME'], 5, "openqrm-kvm-lvm-deployment-cloud-hook.php", "Creating clone of image on storage", "", "", 0, 0, 0);
...
function remove_kvm_lvm_deployment($cloud_image_id) {
	...
	global $RESOURCE_INFO_TABLE;
	global $event;
	$event->log("remove_kvm_lvm_deployment", $_SERVER['REQUEST_TIME'], 5, "openqrm-kvm-lvm-deployment-cloud-hook.php", "Removing image on storage", "", "", 0, 0, 0);
...
function resize_kvm_lvm_deployment($cloud_image_id, $resize_value) {
	...
	global $RESOURCE_INFO_TABLE;
	global $event;
	$event->log("resize_kvm_lvm_deployment", $_SERVER['REQUEST_TIME'], 5, "openqrm-kvm-lvm-deployment-cloud-hook.php", "Resize image on storage", "", "", 0, 0, 0);
...
function create_private_kvm_lvm_deployment($cloud_image_id, $private_disk, $private_image_name) {
	...
	global $RESOURCE_INFO_TABLE;
	global $event;
	$event->log("create_private_kvm_lvm_deployment", $_SERVER['REQUEST_TIME'], 5, "openqrm-kvm-lvm-deployment-cloud-hook.php", "Creating private image on storage", "", "", 0, 0, 0);
