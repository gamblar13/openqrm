...
class local_storage_template_edit
{
/**
...
var $lang = array();
	...
	function init() {
		...
		$this->statfile   = $this->openqrm->get('basedir').'/plugins/local-storage/web/storage/'.$resource->id.'.vg.stat';
	}
	...
	function action() {
		...
		$data = $this->edit();
		if($data !== false) {
			$t = $this->response->html->template($this->tpldir.'/local-storage-template-edit.tpl.php');
