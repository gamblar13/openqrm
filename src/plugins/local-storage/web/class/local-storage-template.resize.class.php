...
class local_storage_template_resize
{
/**
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/local-storage-template-resize.tpl.php');
	...
	function resize() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(!$form->get_errors()) {
				...
				$command  = $this->openqrm->get('basedir').'/plugins/local-storage/bin/openqrm-local-storage resize';
	...
	function set_max() {
		...
		$statfile = $this->openqrm->get('basedir').'/plugins/local-storage/web/storage/'.$this->resource->id.'.vg.stat';
		if ($this->file->exists($statfile)) {
			$lines = explode("\n", $this->file->get_contents($statfile));
			if(count($lines) >= 1) {
				foreach($lines as $line) {
