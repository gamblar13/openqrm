...
class local_storage_controller
{
/**
...
var $lang = array(
	...
	'edit' => array (
		'tab' => 'Edit Local-storage',
		'label' => 'Local Volumes on storage %s',
		'lang_id' => 'ID',
		'lang_name' => 'Name',
	...
	function __construct($openqrm, $response) {
		...
		$this->lang     = $this->user->translate($this->lang, $this->rootdir."/plugins/local-storage/lang", 'local-storage.ini');
	...
	function select( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/local-storage/class/local-storage.select.class.php');
			$controller = new local_storage_select($this->openqrm, $this->response);
	...
	function edit( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
		    require_once($this->rootdir.'/plugins/local-storage/class/local-storage.edit.class.php');
		    $controller                  = new local_storage_edit($this->openqrm, $this->response);
		    $controller->actions_name    = $this->actions_name;
		    $controller->tpldir          = $this->tpldir;
		    $controller->message_param   = $this->message_param;
	...
	function add( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
		    require_once($this->rootdir.'/plugins/local-storage/class/local-storage.add.class.php');
		    $controller                = new local_storage_add($this->openqrm, $this->response, $this);
		    $controller->actions_name  = $this->actions_name;
		    $controller->tpldir        = $this->tpldir;
		    $controller->message_param = $this->message_param;
	...
	function remove( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
		    require_once($this->rootdir.'/plugins/local-storage/class/local-storage.remove.class.php');
		    $controller                  = new local_storage_remove($this->openqrm, $this->response);
		    $controller->actions_name    = $this->actions_name;
		    $controller->tpldir          = $this->tpldir;
		    $controller->message_param   = $this->message_param;
		    $controller->identifier_name = $this->identifier_name;
	...
	function duplicate( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
		    require_once($this->rootdir.'/plugins/local-storage/class/local-storage.clone.class.php');
		    $controller                  = new local_storage_clone($this->openqrm, $this->response);
		    $controller->actions_name    = $this->actions_name;
		    $controller->tpldir          = $this->tpldir;
		    $controller->message_param   = $this->message_param;
		    $controller->identifier_name = $this->identifier_name;
