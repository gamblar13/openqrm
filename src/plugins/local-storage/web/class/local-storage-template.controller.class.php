...
class local_storage_template_controller
{
/**
...
var $lang = array(
	'select' => array (
		'tab' => 'Select local-storage',
		'label' => 'Select local-storage',
		'action_edit' => 'edit',
		'table_name' => 'Name',
		'table_id' => 'Id',
	...
	'edit' => array (
		...
		'label' => 'Select Volume group on local-storage %s',
		'lang_id' => 'ID',
		'lang_name' => 'Name',
		...
		'table_vsize' => 'Vsize',
		'table_vfree' => 'VFree',
		'error_no_xen' => 'Storage %s is not of type local-storage',
		'please_wait' => 'Loading Volume group. Please wait ..',
	),
	'volgroup' => array (
		'tab' => 'Edit Volume group',
		'label' => 'Edit Volume group %s on local-storage %s',
		'lang_id' => 'ID',
		'lang_name' => 'Name',
		'lang_deployment' => 'Deployment',
	...
	'drain' => array (
		'label' => 'Drain Template',
		'msg_removed' => 'Drained Template %s',
		'msg_vm_image_still_in_use' => 'Template %s is still in use',
		'please_wait' => 'Draining Template(s). Please wait ..',
		'canceled' => 'Operation canceled. Please wait ..',
	),
	'grab' => array (
		'tab' => 'Grab resource',
		'label' => 'Select a resource to grab to',
		'action_grab' => 'grab',
		'table_state' => 'State',
		'table_id' => 'Id',
		'table_name' => 'Name',
		...
		'canceled' => 'Operation canceled. Please wait ..',
		'msg' => 'Running grab on resource %s transfering to storage %s',
	),
);
	...
	function __construct($openqrm, $response) {
		...
		$this->lang     = $this->user->translate($this->lang, $this->rootdir."/plugins/local-storage/lang", 'local-storage-template.ini');
	...
	function action($action = null) {
		...
		switch( $this->action ) {
			...
			break;
			case $this->lang['volgroup']['action_drain']:
			case 'drain':
				$content[] = $this->select(false);
				$content[] = $this->edit(false);
				$content[] = $this->volgroup(false);
				$content[] = $this->drain(true);
			break;
		}
	...
	//--------------------------------------------
	function api() {
		require_once($this->rootdir.'/plugins/local-storage/class/local-storage.api.class.php');
		$controller = new local_storage_api($this);
		$controller->action();
	}
	...
	function select( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/local-storage/class/local-storage-template.select.class.php');
			$controller = new local_storage_template_select($this->openqrm, $this->response);
	...
	function edit( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/local-storage/class/local-storage-template.edit.class.php');
			$controller                  = new local_storage_template_edit($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
	...
	function volgroup( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			require_once($this->rootdir.'/plugins/local-storage/class/local-storage-template.volgroup.class.php');
			$controller                  = new local_storage_template_volgroup($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	 ...
	 */
	//--------------------------------------------
	function grab( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/local-storage/class/local-storage-template.grab.class.php');
			$controller                  = new local_storage_template_grab($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
			$controller->prefix_tab      = $this->prefix_tab;
			$controller->lang            = $this->lang['grab'];
			$data = $controller->action();
		}
		$content['label']   = $this->lang['grab']['tab'];
		$content['value']   = $data;
		$content['target']  = $this->response->html->thisfile;
		$content['request'] = $this->response->get_array($this->actions_name, 'grab' );
		$content['onclick'] = false;
		if($this->action === 'grab'){
			$content['active']  = true;
		}
	...
	function add( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/local-storage/class/local-storage-template.add.class.php');
			$controller                = new local_storage_template_add($this->openqrm, $this->response, $this);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function remove( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/local-storage/class/local-storage-template.remove.class.php');
			$controller                  = new local_storage_template_remove($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function snap( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/local-storage/class/local-storage-template.snap.class.php');
			$controller                  = new local_storage_template_snap($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function duplicate( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/local-storage/class/local-storage-template.clone.class.php');
			$controller                  = new local_storage_template_clone($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function resize( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/local-storage/class/local-storage-template.resize.class.php');
			$controller                  = new local_storage_template_resize($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	 ...
	 */
	//--------------------------------------------
	function drain( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/local-storage/class/local-storage-template.drain.class.php');
			$controller                  = new local_storage_template_drain($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
			$controller->lang            = $this->lang['drain'];
			$controller->rootdir         = $this->rootdir;
			$controller->prefix_tab      = $this->prefix_tab;
			$data = $controller->action();
		}
		...
		$content['value']   = $data;
		$content['target']  = $this->response->html->thisfile;
		$content['request'] = $this->response->get_array($this->actions_name, 'drain' );
		$content['onclick'] = false;
		if($this->action === 'drain' || $this->action === $this->lang['volgroup']['action_drain']){
			$content['active']  = true;
		}
		return $content;
	}
	...
	function reload($mode) {
		...
		// reload volume group
		if($mode === 'vg') {
			$file = $OPENQRM_SERVER_BASE_DIR.'/plugins/local-storage/web/storage/'.$resource->id.'.vg.stat';
			$command .= $OPENQRM_SERVER_BASE_DIR.'/plugins/local-storage/bin/openqrm-local-storage post_vg -t '.$deployment->type;
		}
		// reload logical volumes
		if($mode === 'lv') {
			$file = $OPENQRM_SERVER_BASE_DIR.'/plugins/local-storage/web/storage/'.$resource->id.'.'.$volgroup.'.lv.stat';
			$command .= $OPENQRM_SERVER_BASE_DIR.'/plugins/local-storage/bin/openqrm-local-storage post_lv';
