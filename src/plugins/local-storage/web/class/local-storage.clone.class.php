...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/local-storage-clone.tpl.php');
	...
	function duplicate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$local_storage_volume->get_instance_by_name($name);
			if (isset($local_storage_volume->id) && $local_storage_volume->id > 0) {
			...
			if(isset($error)) {
				...
				// add volume
				$local_storage_volume->get_instance_by_name($this->volume);
				$volume_fields = array();
				$volume_fields["local_storage_volume_id"] = openqrm_db_get_free_id('local_storage_volume_id', $local_storage_volume->_db_table);
				$volume_fields['local_storage_volume_name'] = $name;
				$volume_fields['local_storage_volume_root'] = $local_storage_volume->root;
				$volume_fields['local_storage_volume_description'] = $local_storage_volume->description;
				$local_storage_volume->add($volume_fields);
				// add image
