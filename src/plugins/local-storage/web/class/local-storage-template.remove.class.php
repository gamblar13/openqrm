...
class local_storage_template_remove
{
/**
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/local-storage-template-remove.tpl.php');
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($lvols as $key => $lvol) {
					...
					if (count($appliances_using_resource) > 0) {
						...
						$command  = $this->openqrm->get('basedir').'/plugins/local-storage/bin/openqrm-local-storage remove';
						$command .= ' -n '.$lvol;
