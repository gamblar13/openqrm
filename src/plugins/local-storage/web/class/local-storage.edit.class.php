...
var $lang = array();
	...
	function __construct($openqrm, $response) {
		...
		require_once($this->openqrm->get('webdir').'/plugins/local-storage/class/local-storage-volume.class.php');
	}
	...
	function action() {
		...
		$data = $this->edit();
		if($data !== false) {
			$t = $this->response->html->template($this->tpldir.'/local-storage-edit.tpl.php');
	...
	//--------------------------------------------
	function edit() {
		if($this->deployment->type === 'local-storage') {
			...
			$table->sort            = 'local_storage_volume_name';
			$table->limit           = 10;
			$table->offset          = 0;
			$table->order           = 'ASC';
			...
			$local_storage_volume_arr = $local_storage_volume->display_overview($table->offset, $table->limit, $table->sort, $table->order);
			if(count($local_storage_volume_arr) >= 1) {
				foreach($local_storage_volume_arr as $k => $v) {
					...
					$c->css     = 'clone';
					$c->href    = $this->response->get_url($this->actions_name, "clone").'&volume='.$v['local_storage_volume_name'];

					// edit image
					$local_image = new image();
					$local_image->get_instance_by_name($v['local_storage_volume_name']);
					...
					$body[] = array(
						'icon' => $d['icon'],
						'id' => $v['local_storage_volume_id'],
						'name'   => $v['local_storage_volume_name'],
						'root'   => $v['local_storage_volume_root'],
						'description' => $v['local_storage_volume_description'],
						'actions' => $c,
						'edit' => $e,
					);
				}
