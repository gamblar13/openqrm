	...
	function vm_update() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(isset($error)) {
				...
				$vnc_port = 5900 + $vm_id;
				$command = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/vmware-esx/bin/openqrm-vmware-esx update -i ".$this->resource->ip." -n ".$name." -l ".$datastore." -d ".$disk." -m ".$mac." -t ".$type." -c ".$cpu." -r ".$memory." ".$vm_additional_nics." -va ".$vnc." -vp ".$vnc_port." -b ".$bootorder;
