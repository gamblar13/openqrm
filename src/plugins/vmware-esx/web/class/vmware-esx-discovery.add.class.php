...
//		}
	...
	function add() {
		...
		if(!$form->get_errors()	&& $response->submit()) {
			...
			if(!$form->get_errors()) {
				...
				$command  = $this->openqrm->get('basedir')."/plugins/vmware-esx/bin/openqrm-vmware-esx configure -i ".$esx_ip." -eu ".$esx_user." -ep ".$esx_password." -eh ".$esx_hostname;
						...
						foreach($lines as $line) {
							if($line !== '') {
								if (!strstr($line, "success")) {
									$response->msg = sprintf($this->lang['error_integrating'], $this->discovery->vmw_esx_ad_id);
									return $response;
								}
							}
						}
	...
	function get_response() {
		...
		$d['vmware_esx_ad_id']['object']['attrib']['value']  = $this->discovery->vmw_esx_ad_id;
