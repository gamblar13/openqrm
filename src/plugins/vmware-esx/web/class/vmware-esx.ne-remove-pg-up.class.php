...
//			if(!$form->get_errors() && $response->submit()) {
			if($response->submit()) {
				...
				if ($vs_name === 'vSwitch0') {
					...
					$command  = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/vmware-esx/bin/openqrm-vmware-esx-network remove_pg_up -i ".$this->resource->ip." -n ".$vs_name." -p ".$pg_name." -u ".$uplink;
