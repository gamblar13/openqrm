...
var $lang = array(
	...
	'vm_add' => array (
		...
		'error_name' => 'VNC Password must be %s',
		'please_wait' => 'Loading Host configuration. Please wait ..',
	...
	function select( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new vmware_esx_select($this->response);
...
// #############################################################################
	...
	function vm( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_vm()) {
				...
				$controller                  = new vmware_esx_vm($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
	...
	function reload_vm() {
		...
		$command  = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/vmware-esx/bin/openqrm-vmware-esx post_vm_list -i ".$resource->ip;
		$openqrm = new resource();
		$openqrm->get_instance_by_id(0);
		$file = 'vmware-esx-stat/'.$resource->ip.'.vm_list';
		if(file_exists($file)) {
			unlink($file);
		}
	...
	function vm_add( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ds()) {
				...
				$controller                  = new vmware_esx_vm_add($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function vm_update( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ds()) {
				...
				$controller                  = new vmware_esx_vm_update($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function vm_remove( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_vm()) {
				...
				$controller                  = new vmware_esx_vm_remove($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function vm_start( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_vm()) {
				...
				$controller                  = new vmware_esx_vm_start($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function vm_stop( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_vm()) {
				...
				$controller                  = new vmware_esx_vm_stop($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function vm_console( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new vmware_esx_vm_console($this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function vm_boot( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_vm()) {
				...
				$controller                  = new vmware_esx_vm_boot($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
...
// #############################################################################
	...
	function ds( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ds()) {
				...
				$controller                  = new vmware_esx_ds($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function reload_ds() {
		...
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$command  = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/vmware-esx/bin/openqrm-vmware-esx-datastore post_ds_list -i ".$resource->ip;
		$openqrm = new resource();
		$openqrm->get_instance_by_id(0);
		$file = 'vmware-esx-stat/'.$resource->ip.'.ds_list';
		if(file_exists($file)) {
			unlink($file);
		}
		$resource->send_command($openqrm->ip, $command);
	...
	function ds_add_nas( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ds()) {
				...
				$controller                  = new vmware_esx_ds_add_nas($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ds_remove_nas( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ds()) {
				...
				$controller                  = new vmware_esx_ds_remove_nas($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ds_add_iscsi( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ds()) {
				...
				$controller                  = new vmware_esx_ds_add_iscsi($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ds_remove_iscsi( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ds()) {
				...
				$controller                  = new vmware_esx_ds_remove_iscsi($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
...
// #############################################################################
	...
	function ne( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ne()) {
				...
				$controller                  = new vmware_esx_ne($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function reload_ne() {
		...
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$command  = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/vmware-esx/bin/openqrm-vmware-esx-network post_net_config -i ".$resource->ip;
		$openqrm = new resource();
		$openqrm->get_instance_by_id(0);
		$file = 'vmware-esx-stat/'.$resource->ip.'.net_config';
		if(file_exists($file)) {
			unlink($file);
		}
		$resource->send_command($openqrm->ip, $command);
	...
	function ne_add_vs( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ne()) {
				...
				$controller                  = new vmware_esx_ne_add_vs($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ne_remove_vs( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ne()) {
				...
				$controller                  = new vmware_esx_ne_remove_vs($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ne_select_vs( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ne()) {
				...
				$controller                  = new vmware_esx_ne_select_vs($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ne_add_vs_pg( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ne()) {
				...
				$controller                  = new vmware_esx_ne_add_vs_pg($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ne_remove_vs_pg( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ne()) {
				...
				$controller                  = new vmware_esx_ne_remove_vs_pg($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ne_add_vs_up( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ne()) {
				...
				$controller                  = new vmware_esx_ne_add_vs_up($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ne_remove_vs_up( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ne()) {
				...
				$controller                  = new vmware_esx_ne_remove_vs_up($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ne_add_pg_up( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ne()) {
				...
				$controller                  = new vmware_esx_ne_add_pg_up($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ne_remove_pg_up( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ne()) {
				...
				$controller                  = new vmware_esx_ne_remove_pg_up($this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
...
// #############################################################################
	...
	function reboot( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new vmware_esx_reboot($this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function shutdown( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new vmware_esx_shutdown($this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
