	...
	function vm_add() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(isset($error)) {
				...
				$command = $this->openqrm->get('basedir')."/plugins/vmware-esx/bin/openqrm-vmware-esx create -i ".$this->resource->ip." -n ".$name." -l ".$datastore." -d ".$disk." -m ".$mac." -t ".$type." -c ".$cpu." -r ".$memory." ".$vm_additional_nics." -va ".$vnc." -vp ".$vnc_port." -b ".$bootorder;
