...
var $lang = array();
	...
	function ds_add_iscsi() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$command     = $this->openqrm->get('basedir')."/plugins/vmware-esx/bin/openqrm-vmware-esx-datastore add_iscsi -i ".$this->resource->ip." -n ".$name." -t ".$target." -q ".$targetip." -g ".$portgroup." -v ".$vswitch." -k ".$vmk." -ki ".$vmkip." -ks ".$vmksubnet;
			if(isset($error)) {
