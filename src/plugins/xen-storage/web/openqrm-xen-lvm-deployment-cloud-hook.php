...
function create_clone_xen_lvm_deployment($cloud_image_id, $image_clone_name, $disk_size) {
	...
	$image_clone_cmd=$OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/xen-storage/bin/openqrm-xen-storage snap -n ".$image_location_name." -v ".$volume_group." -s ".$image_clone_name." -m ".$disk_size." -t ".$deployment->type;
...
function remove_xen_lvm_deployment($cloud_image_id) {
	...
	$image_remove_clone_cmd=$OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/xen-storage/bin/openqrm-xen-storage remove -n ".$image_location_name." -v ".$volume_group." -t ".$deployment->type;
...
function resize_xen_lvm_deployment($cloud_image_id, $resize_value) {
	...
	$image_resize_cmd=$OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/xen-storage/bin/openqrm-xen-storage resize -n ".$image_location_name." -v ".$volume_group." -m ".$resize_value." -t ".$deployment->type;
...
function create_private_xen_lvm_deployment($cloud_image_id, $private_disk, $private_image_name) {
	...
	$image_resize_cmd=$OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/xen-storage/bin/openqrm-xen-storage clone -n ".$image_location_name." -s ".$private_image_name." -v ".$volume_group." -m ".$private_disk." -u ".$openqrm_admin_user->name." -p ".$openqrm_admin_user->password." -t ".$deployment->type;
