		...
		case 'start':
			if (isset($_REQUEST['identifier_table1'])) {
				...
				foreach($_REQUEST['identifier_table1'] as $xen_name) {
                    ...
                    // send command
                    $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen-storage/bin/openqrm-xen-storage-vm start -n $xen_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                    $xen->send_command($xen->ip, $resource_command);
                    // wait for statfile to appear again
                    if (!wait_for_statfile($statfile)) {
		...
		case 'stop':
			if (isset($_REQUEST['identifier_table1'])) {
				...
				foreach($_REQUEST['identifier_table1'] as $xen_name) {
                    ...
                    // send command
                    $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen-storage/bin/openqrm-xen-storage-vm stop -n $xen_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                    $xen->send_command($xen->ip, $resource_command);
                    // wait for statfile to appear again
                    if (!wait_for_statfile($statfile)) {
		...
		case 'reboot':
			if (isset($_REQUEST['identifier_table1'])) {
				...
				foreach($_REQUEST['identifier_table1'] as $xen_name) {
                    ...
                    // send command
                    $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen-storage/bin/openqrm-xen-storage-vm reboot -n $xen_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                    $xen->send_command($xen->ip, $resource_command);
                    // wait for statfile to appear again
                    if (!wait_for_statfile($statfile)) {
			...
			if (isset($_REQUEST['identifier_table1'])) {
				...
				foreach($_REQUEST['identifier_table1'] as $xen_name) {
                    ...
                    if ("$xen_migrate_type" == "1") {
                        $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen-storage/bin/openqrm-xen-storage-vm migrate -n $xen_name -i $destination->ip -t live -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                    } else {
                        $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen-storage/bin/openqrm-xen-storage-vm migrate -n $xen_name -i $destination->ip -t regular -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                    }
                    $xen->send_command($xen->ip, $resource_command);
                    // wait for statfile to appear again
                    if (!wait_for_statfile($statfile)) {
