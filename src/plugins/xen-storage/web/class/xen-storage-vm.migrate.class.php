...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/xen-storage-vm-migrate.tpl.php');
	...
	function migrate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$destination_command = $this->openqrm->get('basedir').'/plugins/xen-storage/bin/openqrm-xen-storage-vm create_vm_config -n '.$this->vm;
			$destination->send_command($dest_host_resource->ip, $destination_command);
			// give some time
			sleep(4);
			// remove current stat file
			$statfile= $this->openqrm->get('webdir')."/plugins/xen-storage/xen-stat/".$this->vm.".vm_migrated_successfully";
			if (file_exists($statfile)) {
				unlink($statfile);
			}
			// send command
			$xen_migrate_parameter = "-t live";
			$resource_command = $this->openqrm->get('basedir')."/plugins/xen-storage/bin/openqrm-xen-storage-vm migrate -n ".$this->vm." -i ".$destination->ip." ".$xen_migrate_parameter." -u ".$this->openqrm->admin()->name." -p ".$this->openqrm->admin()->password;
