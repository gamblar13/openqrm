...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/xen-storage-vm-add.tpl.php');
	...
	function add() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(count($errors) > 0 || $form->get_errors()) {
				...
				$command  = $this->openqrm->get('basedir').'/plugins/xen-storage/bin/openqrm-xen-storage-vm create';
				$command .= ' -n '.$name;
				$command .= ' -m '.$mac;
