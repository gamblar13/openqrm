...
var $lang = array();
	...
	function __construct($openqrm, $response) {
		...
		$this->statfile  = $this->openqrm->get('basedir').'/plugins/xen-storage/web/kvm-stat/'.$resource->id.'.pick_iso_config';
...
//		echo $response;
	 ...
	 *
	 * for testing
	 * http://cloud/openqrm/base/plugins/xen-storage/index-vm.php?appliance_id=&kvm_storage_vm_action=iso&appliance_id=2&path=/tmp
	 *
	 */
	//--------------------------------------------
	function iso() {
		...
		if ($iso_path !== '') {
		    ...
		    $command  = $this->openqrm->get('basedir')."/plugins/xen-storage/bin/openqrm-xen-storage-vm iso";
		    $command .= ' -q '.$iso_path;
		    $command .= ' -u '.$this->openqrm->admin()->name.' -p '.$this->openqrm->admin()->password;

		    $file = $this->openqrm->get('basedir').'/plugins/xen-storage/web/kvm-stat/'.$this->resource->id.'.pick_iso_config';
			...
			$table = $response->html->tablebuilder('table', $response);
