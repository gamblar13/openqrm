...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/xen-storage-vm-update.tpl.php');
	...
	function update() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(count($errors) > 0 || $form->get_errors()) {
				...
				$command  = $this->openqrm->get('basedir').'/plugins/xen-storage/bin/openqrm-xen-storage-vm update';
				$command .= ' -n '.$this->vm;
	...
	function get_response() {
		...
		$file = $this->openqrm->get('basedir').'/plugins/xen-storage/web/xen-stat/'.$this->resource->id.'.'.$this->vm.'.vm_config';
		$ini  = openqrm_parse_conf($file);

		$file = $this->openqrm->get('basedir').'/plugins/xen-storage/web/xen-stat/'.$this->resource->id.'.bridge_config';
		...
		$this->response->params['mac'] = $ini['OPENQRM_XEN_STORAGE_VM_MAC_1'];
		...
		$d['cpus']['object']['attrib']['selected']  = array($ini['OPENQRM_XEN_STORAGE_VM_CPU']);
		...
		if($ini['OPENQRM_XEN_STORAGE_VM_RAM'] !== '') {
			$d['memory']['object']['attrib']['selected'] = array($ini['OPENQRM_XEN_STORAGE_VM_RAM']);
		}
		...
		$d['mac']['object']['attrib']['value']     = $ini['OPENQRM_XEN_STORAGE_VM_MAC_1'];
		...
		$d['bridge']['object']['attrib']['selected'] = array($ini['OPENQRM_XEN_STORAGE_VM_NET_1']);

		// net 1
		$checked = false;
		if($ini['OPENQRM_XEN_STORAGE_VM_MAC_2'] !== '' ) {
			$mac = $ini['OPENQRM_XEN_STORAGE_VM_MAC_2'];
			$checked = true;
		} else {
		...
		if(isset($ini['OPENQRM_XEN_STORAGE_VM_NET_2']) && $checked === true) {
			$d['bridge1']['object']['attrib']['selected'] = array($ini['OPENQRM_XEN_STORAGE_VM_NET_2']);
		}

		// net 2
		$checked = false;
		if($ini['OPENQRM_XEN_STORAGE_VM_MAC_3'] !== '' ) {
			$mac = $ini['OPENQRM_XEN_STORAGE_VM_MAC_3'];
			$checked = true;
		} else {
			$this->resource->generate_mac();
			$mac = $this->resource->mac;
		...
		if(isset($ini['OPENQRM_XEN_STORAGE_VM_NET_3']) && $checked === true) {
			$d['bridge2']['object']['attrib']['selected'] = array($ini['OPENQRM_XEN_STORAGE_VM_NET_3']);
		}

		// net 3
		$checked = false;
		if($ini['OPENQRM_XEN_STORAGE_VM_MAC_4'] !== '' ) {
			$mac = $ini['OPENQRM_XEN_STORAGE_VM_MAC_4'];
			$checked = true;
		} else {
			$this->resource->generate_mac();
			$mac = $this->resource->mac;
		...
		if(isset($ini['OPENQRM_XEN_STORAGE_VM_NET_4']) && $checked === true) {
			$d['bridge3']['object']['attrib']['selected'] = array($ini['OPENQRM_XEN_STORAGE_VM_NET_4']);
		}

		// net 4
		$checked = false;
		if($ini['OPENQRM_XEN_STORAGE_VM_MAC_5'] !== '' ) {
			$mac = $ini['OPENQRM_XEN_STORAGE_VM_MAC_5'];
			$checked = true;
		} else {
			$this->resource->generate_mac();
			$mac = $this->resource->mac;
		...
		if(isset($ini['OPENQRM_XEN_STORAGE_VM_NET_5']) && $checked === true) {
			$d['bridge4']['object']['attrib']['selected'] = array($ini['OPENQRM_XEN_STORAGE_VM_NET_5']);
		}
		...
		if($ini['OPENQRM_XEN_STORAGE_VM_BOOT'] === 'cdrom') {
		...
		if(strpos($ini['OPENQRM_XEN_STORAGE_VM_BOOT'], 'iso') !== false) {
			$isopath = str_replace('iso:', '',$ini['OPENQRM_XEN_STORAGE_VM_BOOT']); 
		...
		if($ini['OPENQRM_XEN_STORAGE_VM_BOOT'] === 'network') {
		...
		if($ini['OPENQRM_XEN_STORAGE_VM_BOOT'] === 'local') {
	...
	//--------------------------------------------
	function reload() {
		$command  = $this->openqrm->get('basedir').'/plugins/xen-storage/bin/openqrm-xen-storage-vm post_vm_config';
		$command .=  ' -n '.$this->vm;
		$command .= ' -u '.$this->openqrm->admin()->name.' -p '.$this->openqrm->admin()->password;
		$id = $this->response->html->request()->get('appliance_id');
		$appliance = new appliance();
		...
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$file = $this->openqrm->get('basedir').'/plugins/xen-storage/web/xen-stat/'.$resource->id.'.'.$this->vm.'.vm_config';
