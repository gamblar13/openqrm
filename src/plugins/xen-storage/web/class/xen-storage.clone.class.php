...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/xen-storage-clone.tpl.php');
	...
	function duplicate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$command  = $this->openqrm->get('basedir').'/plugins/xen-storage/bin/openqrm-xen-storage clone';
			...
			if(isset($error)) {
				...
				$file = $this->openqrm->get('basedir').'/plugins/xen-storage/web/storage/'.$this->resource->id.'.lvm.'.$name.'.sync_progress';
				if($this->file->exists($file)) {
					$this->file->remove($file);
				}
				$root_device_ident = $this->openqrm->get('basedir').'/plugins/xen-storage/web/storage/'.$this->resource->id.'.'.$name.'.root_device';
