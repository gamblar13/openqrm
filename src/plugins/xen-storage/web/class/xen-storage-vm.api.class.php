...
class xen_storage_vm_api
{
	...
	function filepicker() {
		...
		if ($iso_path !== '') {
		    ...
		    $command  = $this->controller->openqrm->get('basedir')."/plugins/xen-storage/bin/openqrm-xen-storage-vm iso";
		    $command .= ' -q '.$iso_path;
		    $command .= ' -u '.$GLOBALS['OPENQRM_ADMIN']->name.' -p '.$GLOBALS['OPENQRM_ADMIN']->password;

		    $file = 'xen-stat/'.$this->resource->id.'.pick_iso_config';
