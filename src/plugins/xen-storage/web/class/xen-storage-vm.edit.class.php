...
var $lang = array();
	...
	function action() {
		...
		$data = $this->edit();
		if($data !== false) {
			$t = $this->response->html->template($this->tpldir.'/xen-storage-vm-edit.tpl.php');
	...
	function edit() {
		...
		$d['icon'] = '<img width="24" height="24" src="'.$this->openqrm->get('baseurl').'/plugins/xen-storage/img/plugin.png">';
			...
			if(count($lines) >= 1) {
				...
				foreach($lines as $line) {
					if($line !== '') {
						...
						if (strcmp($res_virtualization->type, 'xen-storage-vm')) {
							continue;
						}
						...
						if($state !== '2') {						
							...
							// progressbar
							$t->add(uniqid('b'), 'id');
							$t->add($this->openqrm->get('baseurl').'/plugins/xen-storage/kvm-stat/'.$name.'.vm_migration_progress', 'url');
