...
if(htmlobject_request('xen_config_action') != '' && $OPENQRM_USER->role == "administrator") {
	$event->log("action", $_SERVER['REQUEST_TIME'], 5, "xen-storage-vm-config", "Processing config command", "", "", 0, 0, 0);
	switch (htmlobject_request('xen_config_action')) {
		case 'update_ram':
				...
				$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen-storage/bin/openqrm-xen-storage-vm update_vm_ram -n $xen_name -r $xen_update_ram -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
		...
		case 'update_cpu':
				...
				$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen-storage/bin/openqrm-xen-storage-vm update_vm_cpu -n $xen_name -c $xen_update_cpu -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
				$xen_server->send_command($xen_server->ip, $resource_command);
				// and wait for the resulting statfile
				if (!wait_for_statfile($statfile)) {
		...
		case 'add_vm_net':
				...
				$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen-storage/bin/openqrm-xen-storage-vm add_vm_nic -n $xen_name -x $xen_nic_nr -m $xen_new_nic -z $xen_vm_bridge -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
				$xen_server->send_command($xen_server->ip, $resource_command);
				// and wait for the resulting statfile
				if (!wait_for_statfile($statfile)) {
		...
		case 'remove_vm_net':
				...
				$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen-storage/bin/openqrm-xen-storage-vm remove_vm_nic -n $xen_name -x $xen_nic_nr -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
				$xen_server->send_command($xen_server->ip, $resource_command);
				// and wait for the resulting statfile
				if (!wait_for_statfile($statfile)) {
		...
		case 'update_boot_dev':
				...
				$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen-storage/bin/openqrm-xen-storage-vm setboot_by_name -n $xen_name -b $xen_vm_boot_dev $xen_vm_boot_iso -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
...
function xen_vm_config() {
	...
	$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen-storage/bin/openqrm-xen-storage-vm post_vm_config -n $xen_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
	$xen_server->send_command($xen_server->ip, $resource_command);
	sleep($refresh_delay);
	...
	if (isset($store['OPENQRM_XEN_STORAGE_VM_MAC_2'])) {
	...
	if (isset($store['OPENQRM_XEN_STORAGE_VM_MAC_3'])) {
	...
	if (isset($store['OPENQRM_XEN_STORAGE_VM_MAC_4'])) {
	...
	if (isset($store['OPENQRM_XEN_STORAGE_VM_MAC_5'])) {
