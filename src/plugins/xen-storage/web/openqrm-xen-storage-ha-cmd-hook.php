...
function create_xen_storage_vm($host_resource_id, $name, $mac, $memory, $cpu, $swap, $additional_nic_str) {
	...
	global $RESOURCE_INFO_TABLE;
	global $event;
	$event->log("create_xen_storage_vm", $_SERVER['REQUEST_TIME'], 5, "xen-storage-ha-hook", "Creating Xen-Storage VM $name on Host resource $host_resource_id", "", "", 0, 0, 0);
	...
	$event->log("create_xen_storage_vm", $_SERVER['REQUEST_TIME'], 5, "xen-storage-ha-hook", "Running $vm_create_cmd", "", "", 0, 0, 0);
}



// fences a vm
function fence_xen_storage_vm($host_resource_id, $mac) {
	global $OPENQRM_SERVER_BASE_DIR;
	global $OPENQRM_SERVER_IP_ADDRESS;
	global $OPENQRM_EXEC_PORT;
	global $RESOURCE_INFO_TABLE;
	...
	$host_resource = new resource();
	$host_resource->get_instance_by_id($host_resource_id);
	$event->log("fence_xen_storage_vm", $_SERVER['REQUEST_TIME'], 5, "xen-storage-ha-hook", "Fencing Xen-Storage VM $mac from Host resource $host_resource_id", "", "", 0, 0, 0);
	// we need to have an openQRM server object too since some of the
	// virtualization commands are sent from openQRM directly
	$openqrm = new openqrm_server();
	// send command to fence the vm on the host
	$vm_fence_cmd = "$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen-storage/bin/openqrm-xen-storage-vm fence -m ".$mac;
	$event->log("fence_xen_storage_vm", $_SERVER['REQUEST_TIME'], 5, "xen-storage-ha-hook", "Running $vm_fence_cmd", "", "", 0, 0, 0);
