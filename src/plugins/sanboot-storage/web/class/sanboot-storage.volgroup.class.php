...
var $lang = array();
	...
	function init() {
		...
		$this->statfile = $this->openqrm->get('basedir').'/plugins/sanboot-storage/web/storage/'.$resource->id.'.'.$this->volgroup.'.lv.stat';
	}
	...
	function edit() {
		...
		if(strpos($this->deployment->type, 'iscsi-san-deployment') !== false) {
			...
			// Volgroup info
			$lines = explode("\n", file_get_contents($this->openqrm->get('basedir').'/plugins/sanboot-storage/web/storage/'.$this->resource->id.'.vg.stat'));
			...
			if(file_exists($file)) {
				...
				if(count($lines) >= 1) {
					$disabled = array();
					$t = $this->response->html->template($_SERVER["DOCUMENT_ROOT"].'/openqrm/base/js/openqrm-progressbar.js');
					foreach($lines as $line) {
						if($line !== '') {
							$s    = '&#160;';
							$c    = '&#160;';
							...
							if ($line[4] == "clone_in_progress") {
								...
								// progressbar
								$t->add(uniqid('b'), 'id');
								$t->add('/openqrm/base/plugins/sanboot-storage/storage/'.$this->resource->id.'.sanboot.'.$name.'.sync_progress', 'url');
