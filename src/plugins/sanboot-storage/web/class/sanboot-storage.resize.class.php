...
var $lang = array();
	...
	function resize() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(!$form->get_errors()) {
				...
				$command     = $this->openqrm->get('basedir').'/plugins/sanboot-storage/bin/openqrm-sanboot-storage resize';
	...
	function set_max() {
		...
		$statfile = $this->openqrm->get('basedir').'/plugins/sanboot-storage/web/storage/'.$this->resource->id.'.vg.stat';
		if (file_exists($statfile)) {
			$lines = explode("\n", file_get_contents($statfile));
			if(count($lines) >= 1) {
				foreach($lines as $line) {
