			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($lvols as $key => $lvol) {
					...
					if (count($appliances_using_resource) > 0) {
						...
						$command  = $this->openqrm->get('basedir').'/plugins/sanboot-storage/bin/openqrm-sanboot-storage remove';
						$command .= ' -n '.$lvol;
						$command .= ' -v '.$this->volgroup;
						$command .= ' -t '.$deployment->type;
						$command .= ' -u '.$name.' -p '.$pass;

						$file = $this->openqrm->get('basedir').'/plugins/sanboot-storage/web/storage/'.$resource->id.'.'.$this->volgroup.'.lv.stat';
						if(file_exists($file)) {
							unlink($file);
						}
