        ...
        switch (htmlobject_request('action')) {
            ...
            case 'select':
                if (isset($_REQUEST['identifier'])) {
                    foreach($_REQUEST['identifier'] as $id) {
                        ...
                        // post vg status
                        $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/sanboot-storage/bin/openqrm-sanboot-storage post_vg -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
            ...
            case 'select-vg':
                if (isset($_REQUEST['identifier'])) {
                    foreach($_REQUEST['identifier'] as $sanboot_volume_group) {
                        ...
                        $storage_resource->get_instance_by_id($storage->resource_id);
                        // post lv status
                        $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/sanboot-storage/bin/openqrm-sanboot-storage post_lv -u $OPENQRM_USER->name -p $OPENQRM_USER->password -v $sanboot_volume_group -t $deployment->type";
                        // remove current stat file
                        $storage_resource_id = $storage_resource->id;
            ...
            case 'add':
                ...
                if (!strcmp($storage_deployment->type, "iscsi-san-deployment")) {
                    ...
                    $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/sanboot-storage/bin/openqrm-sanboot-storage add -n $sanboot_lun_name -v $sanboot_volume_group -t $storage_deployment->type -m $sanboot_lun_size -i $image_password -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                } else {
                    $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/sanboot-storage/bin/openqrm-sanboot-storage add -n $sanboot_lun_name -v $sanboot_volume_group -t $storage_deployment->type -m $sanboot_lun_size -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                }
                // remove current stat file
                $storage_resource_id = $storage_resource->id;
                $statfile="storage/".$storage_resource_id.".".$sanboot_volume_group.".lv.stat";
            ...
            case 'remove':
                if (isset($_REQUEST['identifier'])) {
                    ...
                    foreach($_REQUEST['identifier'] as $sanboot_lun_name) {
                        ...
                        $storage_deployment = new deployment();
                        $storage_deployment->get_instance_by_id($storage->type);
                        $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/sanboot-storage/bin/openqrm-sanboot-storage remove -n $sanboot_lun_name -v $sanboot_volume_group -t $storage_deployment->type -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                        // remove current stat file
                        $storage_resource_id = $storage_resource->id;
                        $statfile="storage/".$storage_resource_id.".".$sanboot_volume_group.".lv.stat";
                        if (file_exists($statfile)) {
            ...
            case 'clone':
                ...
                if (strlen($sanboot_lun_snap_name)) {
                    ...
                    if (!strcmp($deployment->type, "iscsi-san-deployment")) {
                        ...
                        // generate a password for the image
                        $image_password = $image->generatePassword(12);
                        $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/sanboot-storage/bin/openqrm-sanboot-storage $sanboot_action -n $sanboot_lun_name -v $sanboot_volume_group -t $deployment->type -s $sanboot_lun_snap_name -m $sanboot_lun_snap_size -i $image_password -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                    } else {
                        $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/sanboot-storage/bin/openqrm-sanboot-storage $sanboot_action -n $sanboot_lun_name -v $sanboot_volume_group -t $deployment->type -s $sanboot_lun_snap_name -m $sanboot_lun_snap_size -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                    }
                    // remove current stat file
                    $storage_resource_id = $storage_resource->id;
                    $statfile="storage/".$storage_resource_id.".".$sanboot_volume_group.".lv.stat";
            ...
            case 'resize':
                ...
                if (!strcmp($deployment->type, "iscsi-san-deployment")) {
                    ...
                    // generate a password for the image
                    $image_password = $image->generatePassword(12);
                    $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/sanboot-storage/bin/openqrm-sanboot-storage resize -n $sanboot_lun_name -v $sanboot_volume_group -t $deployment->type -m $sanboot_lun_resize -i $image_password -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                } else {
                    $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/sanboot-storage/bin/openqrm-sanboot-storage resize -n $sanboot_lun_name -v $sanboot_volume_group -t $deployment->type -m $sanboot_lun_resize -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                }
                // remove current stat file
                $storage_resource_id = $storage_resource->id;
                $statfile="storage/".$storage_resource_id.".".$sanboot_volume_group.".lv.stat";
...
function sanboot_storage_lv_display($sanboot_storage_id, $sanboot_volume_group) {
    ...
    // sanboot table
    $table = new htmlobject_table_builder('sanboot_luns', '', '', '', 'luns');
	$arHead = array();
