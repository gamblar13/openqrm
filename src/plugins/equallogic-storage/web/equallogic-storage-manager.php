...
$equallogic_storage_id = htmlobject_request('equallogic_storage_id');
					...
					if (isset($_REQUEST['identifier'])) {
						foreach($_REQUEST['identifier'] as $id) {
							...
							if (!strlen($eq_storage->storage_id)) {
								...
								$e_deployment->get_instance_by_type("equallogic");
								$image_fields = array();
								$image_fields["image_id"] = openqrm_db_get_free_id('image_id', 'image_info');
								$image_fields['image_name'] = $eq_image_name;
								$image_fields['image_type'] = $e_deployment->type;
								$image_fields['image_rootfstype'] = 'ext3';
								$image_fields['image_storageid'] = $eq_storage->storage_id;
								$image_fields['image_comment'] = "Image Object for volume $eq_image_name";
								$image_fields['image_rootdevice'] = '/dev/'.$eq_storage_ip.'/'.$eq_image_name;
								$image = new image();
					 ...
					 // validate lun name
						  ...
						  $strMsg .= "Got emtpy clone LUN name. Not adding ...";
							  ...
							  exit(0);
								...
								$openqrm_server_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/equallogic-storage/bin/openqrm-equallogic-storage clone -n $lun_name -u $eq_user -p $eq_password -e $eq_storage_ip -s $eq_clone_name -ou $OPENQRM_ADMIN->name -op $OPENQRM_ADMIN->password";
								$output = shell_exec($openqrm_server_command);
								...
								$image_fields = array();
								$image_fields["image_id"] = openqrm_db_get_free_id('image_id', 'image_info');
								$image_fields['image_name'] = $eq_clone_name;
								$image_fields['image_type'] = $e_deployment->type;
								$image_fields['image_rootfstype'] = 'ext3';
								$image_fields['image_storageid'] = $eq_storage->storage_id;
								$image_fields['image_comment'] = "Image Object for volume $eq_clone_name";
								$image_fields['image_rootdevice'] = '/dev/'.$eq_storage_ip.'/'.$eq_clone_name;
								$image = new image();
								$image->add($image_fields);
...
function equallogic_select_storage() {
	...
	foreach ($storage_array as $index => $storage_db) {
		...
		$storage_icon="/openqrm/base/plugins/equallogic-storage/img/plugin.png";
