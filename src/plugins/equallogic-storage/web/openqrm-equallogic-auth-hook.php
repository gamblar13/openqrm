	...
	function storage_auth_function($cmd, $appliance_id) {
		...
		switch($cmd) {
			case "start":
				...
				$auth_start_cmd = "$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/$deployment_plugin_name/bin/openqrm-$deployment_plugin_name auth -n $image_name -r $image_rootdevice -i $image_password -u $eq_user -p $eq_password -e $eq_storage_ip -ou $openqrm_admin_user->name -op $openqrm_admin_user->password";
