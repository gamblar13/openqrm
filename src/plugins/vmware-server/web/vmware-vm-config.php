...
<html lang="en">
<head>
	<title>VMWare Server VM configuration</title>
...
if(htmlobject_request('action') != '') {
    switch (htmlobject_request('action')) {
        case 'update_cpus':
                ...
                $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server/bin/openqrm-vmware-server update_vm_cpus -n $vmware_vm_name -c $vmware_update_cpus -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                // remove current stat file
                $statfile="vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
                if (file_exists($statfile)) {
                    unlink($statfile);
                }
                // send command
                $vmware_server->send_command($vmware_server_resource_ip, $resource_command);
                // and wait for the resulting statfile
                if (!wait_for_statfile($statfile)) {
                    $strMsg .= "Error during update_cpus of VMWare Server vm $vmware_vm_name ! Please check the Event-Log<br>";
                } else {
                    $strMsg .="Updated cpus on VMWare Server vm $vmware_vm_name<br>";
                }
        ...
        case 'update_ram':
                ...
                $vmware_server_resource_id = $vmware_server->id;
                $vmware_server_resource_ip = $vmware_server->ip;
                $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server/bin/openqrm-vmware-server update_vm_ram -n $vmware_vm_name -r $vmware_update_ram -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                // remove current stat file
                $statfile="vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
                if (file_exists($statfile)) {
                    unlink($statfile);
                ...
                // and wait for the resulting statfile
                if (!wait_for_statfile($statfile)) {
                    $strMsg .= "Error during update_ram of VMWare Server vm $vmware_vm_name ! Please check the Event-Log<br>";
                } else {
                    $strMsg .="Updated ram on VMWare Server vm $vmware_vm_name<br>";
                }
                redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
            break;
        ...
        case 'add_vm_net':
                ...
                $vmware_server_resource_id = $vmware_server->id;
                $vmware_server_resource_ip = $vmware_server->ip;
                $resource_command = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/vmware-server/bin/openqrm-vmware-server add_vm_nic -n ".$vmware_vm_name." -x ".$vmware_nic_nr." -m ".$vmware_new_nic." -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                // remove current stat file
                $statfile="vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
                if (file_exists($statfile)) {
                    unlink($statfile);
                ...
                // and wait for the resulting statfile
                if (!wait_for_statfile($statfile)) {
                    $strMsg .= "Error during adding nic to VMWare Server vm $vmware_vm_name ! Please check the Event-Log<br>";
                } else {
                    $strMsg .="Added network card to VMWare Server vm $vmware_vm_name<br>";
                }
                redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
            break;
        ...
        case 'remove_vm_net':
                ...
                $vmware_server_resource_id = $vmware_server->id;
                $vmware_server_resource_ip = $vmware_server->ip;
                $resource_command = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/vmware-server/bin/openqrm-vmware-server remove_vm_nic -n ".$vmware_vm_name." -x ".$vmware_nic_nr." -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                // remove current stat file
                $statfile="vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
                if (file_exists($statfile)) {
                    unlink($statfile);
                ...
                // and wait for the resulting statfile
                if (!wait_for_statfile($statfile)) {
                    $strMsg .= "Error during removing nic of VMWare Server vm $vmware_vm_name ! Please check the Event-Log<br>";
                } else {
                    $strMsg .="Removed network card from VMWare Server vm $vmware_vm_name<br>";
                }
                redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
            break;
        ...
        case 'update_vnc':
                ...
                if (!strlen($vmware_update_auth)) {
                    $strMsg .="Got emtpy VNC password. Skipping update of VMWare Server vm $vmware_vm_name<br>";
                    redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
                }
                if (!strlen($vmware_update_port)) {
                    $strMsg .="Got emtpy VNC port. Skipping update of VMWare Server vm $vmware_vm_name<br>";
                    redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
                }
                ...
                $vmware_server_resource_id = $vmware_server->id;
                $vmware_server_resource_ip = $vmware_server->ip;
                $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server/bin/openqrm-vmware-server update_vm_vnc -n $vmware_vm_name -va $vmware_update_auth -vp $vmware_update_port -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                // remove current stat file
                $statfile="vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
                if (file_exists($statfile)) {
                    unlink($statfile);
                ...
                // and wait for the resulting statfile
                if (!wait_for_statfile($statfile)) {
                    $strMsg .= "Error during update_vnc of VMWare Server vm $vmware_vm_name ! Please check the Event-Log<br>";
                } else {
                    $strMsg .="Updated VNC config on VMWare Server vm $vmware_vm_name<br>";
                }
                redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
            break;
        ...
        case 'remove_vnc':
                ...
                $vmware_server_resource_id = $vmware_server->id;
                $vmware_server_resource_ip = $vmware_server->ip;
                $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server/bin/openqrm-vmware-server remove_vm_vnc -n $vmware_vm_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                // remove current stat file
                $statfile="vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
                if (file_exists($statfile)) {
                    unlink($statfile);
                ...
                // and wait for the resulting statfile
                if (!wait_for_statfile($statfile)) {
                    $strMsg .= "Error during removing VNC config on VMWare Server vm $vmware_vm_name ! Please check the Event-Log<br>";
                } else {
                    $strMsg .="Removed VNC config on VMWare Server vm $vmware_vm_name<br>";
                }
                redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
            break;
        ...
        case 'get_config':
            ...
            $vmware_server_resource_id = $vmware_server->id;
            $vmware_server_resource_ip = $vmware_server->ip;
            $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server/bin/openqrm-vmware-server post_vm_config -n $vmware_vm_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
            // remove current stat file
            $statfile="vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
            if (file_exists($statfile)) {
                unlink($statfile);
            ...
            // and wait for the resulting statfile
            if (!wait_for_statfile($statfile)) {
                $strMsg = "Error refreshing config of VMWare Server vm $vmware_vm_name ! Please check the Event-Log<br>";
            } else {
                $strMsg = "";
            }
            redirect_config($strMsg, $vmware_server_id, $vmware_vm_name);
...
function vmware_vm_config() {
    ...
    $vmware_server_resource_ip = $vmware_server->ip;
    $vmware_server_resource_id = $vmware_server->id;
	$vmware_vm_conf_file=$OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/vmware-server/web/vmware-server-stat/".$vmware_server_resource_id.".".$vmware_vm_name.".vm_config";
    ...
    // backlink
    $backlink = "<a href='vmware-server-manager.php?vmware_server_id=".$vmware_server_id."'>back</a>";
...
function vmware_vm_config_net() {
	...
	if ($nic_number < 4) {
		...
		$vm_config_add_nic_disp .= htmlobject_input('vmware_new_nic', array("value" => $suggested_vmware_mac, "label" => 'Add Network'), 'text', 10);
...
if ($OPENQRM_USER->role == "administrator") {
	...
	if ("$vmware_component" == "ram") {
		$output[] = array('label' => 'VMWare Server Configure VM', 'value' => vmware_vm_config_ram());
	} else if ("$vmware_component" == "cpus") {
		$output[] = array('label' => 'VMWare Server Configure VM', 'value' => vmware_vm_config_cpus());
	} else if ("$vmware_component" == "net") {
		$output[] = array('label' => 'VMWare Server Configure VM', 'value' => vmware_vm_config_net());
	} else if ("$vmware_component" == "vnc") {
		$output[] = array('label' => 'VMWare Server Configure VM', 'value' => vmware_vm_config_vnc());
	} else {
		$output[] = array('label' => 'VMWare Server Configure VM', 'value' => vmware_vm_config());
	}
}
