...
function create_vmware_server_vm($host_resource_id, $name, $mac, $memory, $cpu, $swap, $additional_nic_str) {
	global $OPENQRM_SERVER_BASE_DIR;
	global $OPENQRM_SERVER_IP_ADDRESS;
	...
	global $vmware_mac_address_space;
	global $event;
	$event->log("create_vmware_server_vm", $_SERVER['REQUEST_TIME'], 5, "vmware-server-cloud-hook", "Creating VMware-Server VM $name on Host resource $host_resource_id", "", "", 0, 0, 0);
	...
	$event->log("create_vmware_server_vm", $_SERVER['REQUEST_TIME'], 5, "vmware-server-cloud-hook", "Running $vm_create_cmd", "", "", 0, 0, 0);
...
// removes a cloud vm
function remove_vmware_server_vm($host_resource_id, $name, $mac) {
	global $OPENQRM_SERVER_BASE_DIR;
	global $OPENQRM_SERVER_IP_ADDRESS;
	global $OPENQRM_EXEC_PORT;
	global $RESOURCE_INFO_TABLE;
	...
	$host_resource = new resource();
	$host_resource->get_instance_by_id($host_resource_id);
	$event->log("remove_vmware_server_vm", $_SERVER['REQUEST_TIME'], 5, "vmware-server-cloud-hook", "Removing VMware-Server VM $name/$mac from Host $host_resource_id", "", "", 0, 0, 0);
	// we need to have an openQRM server object too since some of the
	// virtualization commands are sent from openQRM directly
	$openqrm = new openqrm_server();
	// send command to create the vm on the host
	$vm_remove_cmd = "$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vmware-server/bin/openqrm-vmware-server delete -n ".$name;
	$event->log("remove_vmware_server_vm", $_SERVER['REQUEST_TIME'], 5, "vmware-server-cloud-hook", "Running $vm_remove_cmd", "", "", 0, 0, 0);
