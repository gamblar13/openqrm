...
var $lang = array(
	...
	function __construct($user) {
		...
		$this->html->lang = $this->user->translate($this->html->lang, $this->rootdir."/plugins/lvm-storage/lang", 'htmlobjects.ini');
	...
	function select( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new lvm_storage_select($this->response);
	...
	function edit( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new lvm_storage_edit($this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
	...
	function volgroup( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new lvm_storage_volgroup($this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function add( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                = new lvm_storage_add($this->response);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function remove( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new lvm_storage_remove($this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function snap( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new lvm_storage_snap($this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function duplicate( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new lvm_storage_clone($this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function resize( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new lvm_storage_resize($this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function reload($mode) {
		...
		// reload volume group
		if($mode === 'vg') {
			$file = 'storage/'.$resource->id.'.vg.stat';
			$command .= $OPENQRM_SERVER_BASE_DIR.'/openqrm/plugins/lvm-storage/bin/openqrm-lvm-storage post_vg';
		}
		// reload logical volumes
		if($mode === 'lv') {
			$file = 'storage/'.$resource->id.'.'.$volgroup.'.lv.stat';
			$command .= $OPENQRM_SERVER_BASE_DIR.'/openqrm/plugins/lvm-storage/bin/openqrm-lvm-storage post_lv';
