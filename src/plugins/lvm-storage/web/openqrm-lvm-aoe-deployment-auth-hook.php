	...
	function storage_auth_function($cmd, $appliance_id) {
		...
		switch($cmd) {
			case "start":
				...
				$auth_start_cmd = "$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/$deployment_plugin_name/bin/openqrm-$deployment_plugin_name auth -n $image_name -r $image_rootdevice -i $resource_mac -t lvm-aoe-deployment -u $openqrm_admin_user->name -p $openqrm_admin_user->password";
