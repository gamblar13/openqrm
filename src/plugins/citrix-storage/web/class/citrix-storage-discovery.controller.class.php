...
class citrix_storage_discovery_controller
{
/**
...
var $lang = array(
	'select' => array (
		...
		'table_type' => 'Type',
		'error_no_storage' => '<b>No XenServer Host appliance configured yet!</b><br><br>Please create a Citrix XenServer Host first!',
		'new_storage' => 'New Appliance',
	...
	function __construct($openqrm, $response) {
		...
		$this->lang     = $this->user->translate($this->lang, $this->rootdir."/plugins/citrix-storage/lang", 'citrix-storage-discovery.ini');
		$this->tpldir   = $this->rootdir.'/plugins/citrix-storage/tpl';
		require_once $this->rootdir."/plugins/citrix-storage/class/citrix-storage-discovery.class.php";
		$this->discovery = new citrix_storage_discovery();
	}
	...
	function action($action = null) {
		...
		$citrix_storage_discovery = new citrix_storage_discovery();
...
//		}
	...
	function select( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/citrix-storage/class/citrix-storage-discovery.select.class.php');
			$controller = new citrix_storage_discovery_select($this->openqrm, $this->response);
	...
	//--------------------------------------------
	/**
	 * Edit citrix-storage Host for integration
	 *
	 * @access public
	 * @param bool $hidden
	 * @return array
	...
	function edit( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/citrix-storage/class/citrix-storage-discovery.edit.class.php');
			$controller                  = new citrix_storage_discovery_edit($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
	...
	//--------------------------------------------
	/**
	 * Remove citrix-storage Host from discovery
	 *
	 * @access public
	 * @param bool $hidden
	 * @return array
	...
	function delete( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/citrix-storage/class/citrix-storage-discovery.delete.class.php');
			$controller                  = new citrix_storage_discovery_delete($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function add( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/citrix-storage/class/citrix-storage-discovery.add.class.php');
			$controller                = new citrix_storage_discovery_add($this->openqrm, $this->response);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function rescan() {
		require_once $this->rootdir."/plugins/citrix-storage/class/citrix-storage-discovery.class.php";
		$citrix_storage_discovery = new citrix_storage_discovery();
		$command  = $this->basedir."/plugins/citrix-storage/bin/openqrm-citrix-storage-autodiscovery";
		$resource = new resource();
		$resource->get_instance_by_id(0);
		$file = $this->rootdir.'/plugins/citrix-storage/citrix-storage-stat/autodiscovery_finished';
		if(file_exists($file)) {
			unlink($file);
		}
				...
				foreach($lines as $line) {
					if($line !== '') {
						...
						if ((strlen($esx_mac)) && (strlen($esx_ip))) {
							if (($citrix_storage_discovery->mac_discoverd_already($esx_mac)) && ($citrix_storage_discovery->ip_discoverd_already($esx_ip))) {
								$esx_comment = "Added by auto-discovery";
								$citrix_storage_discovery_fields['xenserver_ad_mac'] = $esx_mac;
								$citrix_storage_discovery_fields['xenserver_ad_ip'] = $esx_ip;
								$citrix_storage_discovery_fields['xenserver_ad_hostname'] = $esx_ip;
								$citrix_storage_discovery_fields['xenserver_ad_user'] = "root";
								$citrix_storage_discovery_fields['xenserver_ad_password'] = "";
								$citrix_storage_discovery_fields['xenserver_ad_comment'] = $esx_comment;
								$citrix_storage_discovery_fields['xenserver_ad_is_integrated '] = 0;
								$citrix_storage_discovery->add($citrix_storage_discovery_fields);
							}
							unset($esx_mac);
							unset($esx_ip);
						}
