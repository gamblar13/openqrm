...
class citrix_storage_ds_add_iscsi
{
/**
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/citrix-storage-ds-add-iscsi.tpl.php');
	...
	function ds_add_iscsi() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$command     = $this->openqrm->get('basedir')."/plugins/citrix-storage/bin/openqrm-citrix-storage-datastore add_iscsi -i ".$this->resource->ip." -n ".$name." -t ".$target." -q ".$targetip." ".$iscsi_chap_authentication;
			if(isset($error)) {
