...
var $lang = array();
	...
	function init() {
		...
		$this->statfile = $this->rootdir.'/plugins/citrix-storage/citrix-storage-stat/'.$resource->ip.'.ds_list';
	}
	...
	function action() {
		...
		$data = $this->ds();
		if($data !== false) {
			$t = $this->response->html->template($this->tpldir.'/citrix-storage-ds.tpl.php');
	...
	function ds() {
		...
		if($this->virtualization->type === 'citrix-storage') {
			...
			$table = $this->response->html->tablebuilder('ds_list', $this->response->get_array($this->actions_name, 'ds'));
			$table->sort            = 'name';
			$table->limit           = 20;
			$table->offset          = 0;
			$table->order           = 'ASC';
