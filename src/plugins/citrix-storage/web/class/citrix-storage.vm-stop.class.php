...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/citrix-storage-vm-stop.tpl.php');
		...
		if( $data !== '' ) {
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($data as $key => $vm) {
					$command  = $this->openqrm->get('basedir')."/plugins/citrix-storage/bin/openqrm-citrix-storage-vm stop -i ".$this->resource->ip." -n ".$vm;
	...
	function get_response() {
		...
		$form = $response->get_form($this->actions_name, 'vm_stop');
		$response->form = $form;
		return $response;
	}
