...
class citrix_storage_vm_console
{
/**
...
var $lang = array();
	...
	function vm_console() {
		...
		if(!strlen($plugin_remote_console_function)) {
			...
			$statfile_vm_console = $this->rootdir.'/plugins/citrix-storage/citrix-storage-stat/'.$this->resource->ip.'.'.$vm_name.'.vm_console';
			if(file_exists($statfile_vm_console)) {
				unlink($statfile_vm_console);
			}
			$command = $this->openqrm->get('basedir')."/plugins/citrix-storage/bin/openqrm-citrix-storage-vm start_vnc -i ".$this->resource->ip." -n ".$vm_name." -x ".$vm_id;
			...
			$host_config = $this->openqrm->get('basedir').'/plugins/citrix-storage/conf/host/'.$this->resource->ip.'.pwd';
			...
			$tunnel_command = $this->openqrm->get('basedir')."/plugins/citrix-storage/bin/openqrm-citrix-tunnel ".$this->resource->ip." ".$vm_vnc_console_port_local." ".$vm_vnc_console_port_full;
