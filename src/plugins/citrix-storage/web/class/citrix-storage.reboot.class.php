...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/citrix-storage-vm-reboot.tpl.php');
		...
		if( $data !== '' ) {
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($data as $key => $citrix) {
					...
					$command  = $this->openqrm->get('basedir')."/plugins/citrix-storage/bin/openqrm-citrix-storage host_reboot -i ".$citrix_host_resource->ip;
 					$this->resource->send_command($this->openqrm_server->ip, $command);
					$form->remove($this->identifier_name.'['.$key.']');
					$message[] = sprintf($this->lang['msg_rebooted'], $citrix_host_appliance->name);
				}
				if(count($errors) === 0) {
