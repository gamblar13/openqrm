...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/citrix-storage-vm-add.tpl.php');
	...
	function vm_add() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(isset($error)) {
				...
				$command = $this->openqrm->get('basedir')."/plugins/citrix-storage/bin/openqrm-citrix-storage-vm create -i ".$this->resource->ip." -n ".$name." -l ".$datastore." -m ".$mac." -t ".$template." -c ".$cpu." -r ".$memory." ".$vm_additional_nics." -b ".$bootorder;
