...
class citrix_storage_vdi_select
{
/**
...
var $lang = array();
	...
	function action() {
		$table = $this->select();
		$t = $this->response->html->template($this->tpldir.'/citrix-storage-vdi-select.tpl.php');
	...
	function select() {
		...
		if(count($storages) >= 1) {
			foreach($storages as $k => $v) {
				...
				if($deployment->storagetype === 'citrix-storage') {
			...
			$table = $this->response->html->tablebuilder('citrix', $this->response->get_array($this->actions_name, 'select'));
