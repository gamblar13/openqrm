					...
					foreach($_REQUEST['identifier'] as $id) {
						...
						if (!strlen($na_storage->storage_id)) {
							...
							if (!wait_for_statfile($statfile)) {
								...
								$e_deployment->get_instance_by_type("netapp-deployment");
								$image_fields = array();
								$image_fields["image_id"] = openqrm_db_get_free_id('image_id', 'image_info');
								$image_fields['image_name'] = $na_image_name;
								$image_fields['image_type'] = $e_deployment->type;
								$image_fields['image_rootfstype'] = 'ext3';
								$image_fields['image_storageid'] = $na_storage->storage_id;
								$image_fields['image_comment'] = "Image Object for volume $na_image_name";
								$image_fields['image_rootdevice'] = $na_image_name;
								$image = new image();
				...
				if (strlen($netapp_storage_image_clone_name)) {
					...
					if (!strlen($na_storage->storage_id)) {
						...
						if (!wait_for_statfile($statfile)) {
							...
							$image_fields = array();
							$image_fields["image_id"] = openqrm_db_get_free_id('image_id', 'image_info');
							$image_fields['image_name'] = $netapp_storage_image_clone_name;
							$image_fields['image_type'] = $e_deployment->type;
							$image_fields['image_rootfstype'] = 'ext3';
							$image_fields['image_storageid'] = $netapp_storage_id;
							$image_fields['image_comment'] = "Image Object for volume $netapp_storage_image_clone_name";
							$image_fields['image_rootdevice'] = $netapp_storage_image_clone_name;
							$image = new image();
							$image->add($image_fields);
...
function netapp_select_storage() {
	...
	foreach ($storage_array as $index => $storage_db) {
		...
		$storage_icon="/openqrm/base/plugins/netapp-storage/img/plugin.png";
...
function netapp_display($netapp_storage_id) {
	...
	// na config
	$storage_configuration="<a href=\"netapp-storage-config.php?storage_id=$netapp_storage_id\"><img src=\"/openqrm/base/img/plugin.png\" width=\"24\" height=\"24\" border=\"0\" alt=\"configuration\"/></a>";
	// na admin
	$storage_admin = "<a href=\"http://$storage_resource->ip/na_admin/\"><img src=\"/openqrm/base/img/user.png\" width=\"24\" height=\"24\" border=\"0\" alt=\"administration\"/></a>";
