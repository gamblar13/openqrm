...
var filepicker = {
	init : function() {
		...
		$.ajax({
			url: "api-vm.php?appliance_id={appliance_id}&{actions_name}=filepicker&path=/",
			dataType: "text",
	...
	browse : function(target) {
		document.getElementById('canvas').innerHTML = '<img src="{baseurl}/img/loading.gif" style="margin-top:150px;">';
		$.ajax({
			url: "api-vm.php?appliance_id={appliance_id}&{actions_name}=filepicker&path="+target,
			dataType: "text",
			success: function(response) {
				document.getElementById('canvas').innerHTML = response;	
			}
