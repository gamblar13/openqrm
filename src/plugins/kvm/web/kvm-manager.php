			...
			case 'start':
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $kvm_server_name) {
						...
						$kvm_server = new resource();
						$kvm_server->get_instance_by_id($kvm_appliance->resources);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/kvm/bin/openqrm-kvm start -n $kvm_server_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$kvm_server_resource_id = $kvm_server->id;
						$statfile="kvm-stat/".$kvm_server_resource_id.".vm_list";
						if (file_exists($statfile)) {
			...
			case 'stop':
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $kvm_server_name) {
						...
						foreach ($enabled_plugins as $index => $plugin_name) {
							...
							if (file_exists($plugin_remote_console_hook)) {
								if (file_exists($plugin_remote_console_running)) {
									$event->log("console", $_SERVER['REQUEST_TIME'], 5, "kvm-manager.php", "Found plugin $plugin_name providing a remote console.", "", "", 0, 0, $kvm_resource->id);
						...
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/kvm/bin/openqrm-kvm stop -n $kvm_server_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$kvm_server_resource_id = $kvm_server->id;
						$statfile="kvm-stat/".$kvm_server_resource_id.".vm_list";
						if (file_exists($statfile)) {
			...
			case 'restart':
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $kvm_server_name) {
						...
						$kvm_server = new resource();
						$kvm_server->get_instance_by_id($kvm_appliance->resources);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/kvm/bin/openqrm-kvm reboot -n $kvm_server_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$kvm_server_resource_id = $kvm_server->id;
						$statfile="kvm-stat/".$kvm_server_resource_id.".vm_list";
						if (file_exists($statfile)) {
			...
			case 'delete':
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $kvm_server_name) {
						...
						$kvm_server = new resource();
						$kvm_server->get_instance_by_id($kvm_appliance->resources);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/kvm/bin/openqrm-kvm delete -n $kvm_server_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$kvm_server_resource_id = $kvm_server->id;
						$statfile="kvm-stat/".$kvm_server_resource_id.".vm_list";
						if (file_exists($statfile)) {
			...
			case 'migrate':
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $kvm_server_name) {
						...
						$resource_command_destination = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/kvm/bin/openqrm-kvm start_as_incoming -n ".$kvm_server_name." -j ".$kvm_vm_id." -u ".$OPENQRM_ADMIN->name." -p ".$OPENQRM_ADMIN->password;
						...
						$kvm_server = new resource();
						$kvm_server->get_instance_by_id($kvm_appliance->resources);
						$resource_command=$OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/kvm/bin/openqrm-kvm migrate -n ".$kvm_server_name." -j ".$kvm_vm_id." -k ".$kvm_destination_host_resource_ip." -u ".$OPENQRM_ADMIN->name." -p ".$OPENQRM_ADMIN->password;
						$kvm_server->send_command($kvm_server->ip, $resource_command);
						// wait for statfile to appear again
						if (!wait_for_statfile($statfile, true)) {
							$strMsg .= "Error while migrating KVM VM $kvm_server_name to Host resource $kvm_destination_host_resource_id! Please check the Event-Log<br>";
							continue;
						} else {
							$strMsg .= "Migrated KVM VM ".$kvm_server_name." from Host resource ".$kvm_server->id. " to Host resource ".$kvm_destination_host_resource_id." <br>";
							unlink($statfile);
						}
...
function kvm_server_display($appliance_id) {
	...
	// table 1
	$table1 = new htmlobject_table_builder('kvm_vm_name', '', '', '', 'vms');
	$arHead1 = array();
		...
		foreach ($kvm_server_vm_list_content as $index => $kvm_vm) {
			...
			if (!strstr($kvm_vm, "#")) {
				...
				$arBody1[] = array(
					...
					'kvm_vm_memory' => "<nobr>".$kvm_vm_memory." MB</nobr>",
...
if(htmlobject_request('action') != '') {
	if (isset($_REQUEST['identifier'])) {
		switch (htmlobject_request('action')) {
			case 'select':
				foreach($_REQUEST['identifier'] as $id) {
					$output[] = array('label' => 'KVM VM Manager', 'value' => kvm_server_display($id));
				}
				break;
			case 'reload':
				foreach($_REQUEST['identifier'] as $id) {
		...
		$output[] = array('label' => 'KVM VM Manager', 'value' => kvm_server_select());
	}
} else if (strlen($kvm_server_id)) {
	$output[] = array('label' => 'KVM VM Manager', 'value' => kvm_server_display($kvm_server_id));
} else  {
	$output[] = array('label' => 'KVM VM Manager', 'value' => kvm_server_select());
}
