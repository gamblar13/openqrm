...
var $lang = array();
	...
	function update() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(count($errors) > 0 || $form->get_errors()) {
				...
				$command  = $this->openqrm->get('basedir').'/plugins/kvm/bin/openqrm-kvm update';
				$command .= ' -n '.$this->vm;
	...
	function get_response() {
		...
		// debug: echo '<b>ini</b><br>';
		// debug: $this->response->html->help($ini);
		$bridges = array();
		...
		$d['swap']['object']['attrib']['selected']= array($ini['OPENQRM_KVM_VM_DISK_SIZE_4']);
	...
	//--------------------------------------------
	function reload() {
		$command  = $this->openqrm->get('basedir').'/plugins/kvm/bin/openqrm-kvm post_vm_config';
		$command .=  ' -n '.$this->vm;
		$command .= ' -u '.$this->openqrm->admin()->name.' -p '.$this->openqrm->admin()->password;
		$id = $this->response->html->request()->get('appliance_id');
		$appliance = new appliance();
