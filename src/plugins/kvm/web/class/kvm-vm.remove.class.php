								...
								if($line !== '') {
									...
									if($vm === $line[1]) {
										...
										if (count($appliances_using_resource) > 0) {
											...
											$command  = $this->openqrm->get('basedir').'/plugins/kvm/bin/openqrm-kvm delete -n '.$vm;
											...
											foreach ($enabled as $index => $name) {
												...
												if (file_exists($hook)) {
													if (file_exists($running)) {
														$event->log("console", $_SERVER['REQUEST_TIME'], 5, "kvm-vm.remove.class.php", 'Stopping '.$name.' remote console.', "", "", 0, 0, $kvm->id);
														require_once($hook);
