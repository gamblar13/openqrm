...
var $lang = array();
	...
	function __construct($openqrm, $response, $controller) {
		...
		$this->statfile  = 'kvm-stat/'.$resource->id.'.vm_list';
	}
	...
	function add() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(count($errors) > 0 || $form->get_errors()) {
				...
				$command  = $this->openqrm->get('basedir').'/plugins/kvm/bin/openqrm-kvm create';
				$command .= ' -n '.$name;
				$command .= ' -m '.$mac;
	...
	function get_response() {
		...
		$file = $this->openqrm->get('basedir').'/plugins/kvm/web/kvm-stat/'.$this->resource->id.'.bridge_config';
