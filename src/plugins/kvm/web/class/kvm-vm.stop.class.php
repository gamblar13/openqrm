	...
	function stop() {
		...
		if( $vms !== '' ) {
			...
			foreach($vms as $key => $vm) {
				...
				$command  = $this->openqrm->get('basedir').'/plugins/kvm/bin/openqrm-kvm stop -n '.$vm;
						...
						foreach ($enabled as $index => $name) {
							...
							if (file_exists($hook)) {
								if (file_exists($running)) {
									$event->log("console", $_SERVER['REQUEST_TIME'], 5, "kvm-vm.stop.class.php", 'Found plugin '.$name.' providing a remote console.', "", "", 0, 0, $resource->id);
									require_once($hook);
