...
var $lang = array(
	'select' => array (
		'tab' => 'Select TmpFs-storage',
		'label' => 'Select TmpFs-storage',
		'action_edit' => 'edit',
		'table_name' => 'Name',
		'table_id' => 'Id',
		...
		'error_no_storage' => '<b>No storage configured yet!</b><br><br>Please create a TmpFs Storage first!',
		'new_storage' => 'New Storage',
		'please_wait' => 'Loading Storage. Please wait ..',
	), 
	'edit' => array (
		'tab' => 'Edit TmpFs-storage',
		'label' => 'TmpFs Volumes on storage %s',
		'lang_id' => 'ID',
		'lang_name' => 'Name',
	...
	'add' => array (
		'tab' => 'Add TmpFs Volume',
		'label' => 'Add new Volume',
		'form_add' => 'New TmpFs Volume',
		'form_name' => 'Name',
		'form_size' => 'Size',
	...
	function select( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new tmpfs_storage_select($this->openqrm, $this->response);
	...
	function edit( $hidden = true ) {
		...
		if( $hidden === true ) {
		    ...
		    $controller                  = new tmpfs_storage_edit($this->openqrm, $this->response);
		    $controller->actions_name    = $this->actions_name;
		    $controller->tpldir          = $this->tpldir;
		    $controller->message_param   = $this->message_param;
	...
	function add( $hidden = true ) {
		...
		if( $hidden === true ) {
		    ...
		    $controller                = new tmpfs_storage_add($this->openqrm, $this->response, $this);
		    $controller->actions_name  = $this->actions_name;
		    $controller->tpldir        = $this->tpldir;
		    $controller->message_param = $this->message_param;
	...
	function remove( $hidden = true ) {
		...
		if( $hidden === true ) {
		    ...
		    $controller                  = new tmpfs_storage_remove($this->openqrm, $this->response);
		    $controller->actions_name    = $this->actions_name;
		    $controller->tpldir          = $this->tpldir;
		    $controller->message_param   = $this->message_param;
		    $controller->identifier_name = $this->identifier_name;
	...
	function duplicate( $hidden = true ) {
		...
		if( $hidden === true ) {
		    ...
		    $controller                  = new tmpfs_storage_clone($this->openqrm, $this->response);
		    $controller->actions_name    = $this->actions_name;
		    $controller->tpldir          = $this->tpldir;
		    $controller->message_param   = $this->message_param;
		    $controller->identifier_name = $this->identifier_name;
