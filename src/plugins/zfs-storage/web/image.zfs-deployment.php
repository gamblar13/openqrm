...
function zfs_deployment_wait_for_identfile($sfile) {
	$refresh_delay=1;
	$refresh_loop_max=20;
	$refresh_loop=0;
...
function get_zfs_deployment_image_rootdevice_identifier($zfs_storage_id) {
	global $OPENQRM_SERVER_BASE_DIR;
	...
	// send command
	$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/zfs-storage/bin/openqrm-zfs-storage post_identifier -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
	$storage_resource->send_command($storage_resource->ip, $resource_command);
	if (!zfs_deployment_wait_for_identfile($ident_file)) {
...
function get_zfs_deployment_image_default_rootfs() {
	return "ext3";
}

function get_zfs_deployment_rootfs_transfer_methods() {
	return true;
}

function get_zfs_deployment_rootfs_set_password_method() {
	return true;
}

function get_zfs_deployment_is_network_deployment() {
	return true;
}

function get_zfs_deployment_local_deployment_enabled() {
	return false;
}
