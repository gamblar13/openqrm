			...
			switch (htmlobject_request('action')) {
				case 'refresh':
					if (isset($_REQUEST['identifier'])) {
						foreach($_REQUEST['identifier'] as $id) {
							...
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/zfs-storage/bin/openqrm-zfs-storage post_zpools -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
				...
				case 'select-zpool':
					if (isset($_REQUEST['identifier'])) {
						foreach($_REQUEST['identifier'] as $zpool_name) {
							...
							$storage_resource = new resource();
							$storage_resource->get_instance_by_id($storage->resource_id);
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/zfs-storage/bin/openqrm-zfs-storage post_luns -z $zpool_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
							// remove current stat file
							$storage_resource_id = $storage_resource->id;
				...
				case 'add':
					if (isset($_REQUEST['zfs_lun_name'])) {
						...
						$storage_resource = new resource();
						$storage_resource->get_instance_by_id($storage->resource_id);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/zfs-storage/bin/openqrm-zfs-storage add -n $zfs_lun_name -m $zfs_lun_size -i $zfs_chap_password -z $zpool_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$storage_resource_id = $storage_resource->id;
						$statfile="storage/".$storage_resource_id.".zfs.luns.stat";
						if (file_exists($statfile)) {
						...
						if (!wait_for_statfile($statfile)) {
							...
							// add image
							$e_deployment = new deployment();
							$e_deployment->get_instance_by_type("zfs-deployment");
							$image_fields = array();
							$image_fields["image_id"] = openqrm_db_get_free_id('image_id', 'image_info');
							$image_fields['image_name'] = $zfs_lun_name;
							$image_fields['image_type'] = $e_deployment->type;
							$image_fields['image_rootfstype'] = 'ext3';
							$image_fields['image_storageid'] = $zfs_storage_id;
							$image_fields['image_comment'] = "Image Object for volume $zfs_lun_name";
							$image_fields['image_rootdevice'] = '/dev/'.$zpool_name.'/'.$zfs_lun_name;
							$image = new image();
				...
				case 'remove':
					if (isset($_REQUEST['identifier'])) {
						...
						foreach($_REQUEST['identifier'] as $zfs_lun_name) {
							...
							$storage_resource = new resource();
							$storage_resource->get_instance_by_id($storage->resource_id);
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/zfs-storage/bin/openqrm-zfs-storage remove -n $zfs_lun_name -z $zpool_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
							// remove current stat file
							$storage_resource_id = $storage_resource->id;
							$statfile="storage/".$storage_resource_id.".zfs.luns.stat";
							if (file_exists($statfile)) {
							...
							if (!wait_for_statfile($statfile)) {
								...
								$image = new image();
								$image->remove_by_name($zfs_lun_name);
				...
				case 'snap':
					if (strlen($zpool_lun_snap_name)) {
						...
						$storage_resource = new resource();
						$storage_resource->get_instance_by_id($storage->resource_id);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/zfs-storage/bin/openqrm-zfs-storage snap -n $zpool_lun_name -i $zfs_chap_password -z $zpool_name -s $zpool_lun_snap_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$storage_resource_id = $storage_resource->id;
						$statfile="storage/".$storage_resource_id.".zfs.luns.stat";
						if (file_exists($statfile)) {
						...
						if (!wait_for_statfile($statfile)) {
							...
							$image_fields = array();
							$image_fields["image_id"] = openqrm_db_get_free_id('image_id', 'image_info');
							$image_fields['image_name'] = $zpool_lun_snap_name;
							$image_fields['image_type'] = $e_deployment->type;
							$image_fields['image_rootfstype'] = 'ext3';
							$image_fields['image_storageid'] = $zfs_storage_id;
							$image_fields['image_comment'] = "Image Object for volume $zpool_lun_snap_name";
							$image_fields['image_rootdevice'] = '/dev/'.$zpool_name.'/'.$zpool_lun_snap_name;
							$image = new image();
							$image->add($image_fields);
...
function zfs_select_storage() {
	...
	foreach ($storage_array as $index => $storage_db) {
		...
		$storage_icon="/openqrm/base/plugins/zfs-storage/img/plugin.png";
...
function zfs_storage_zpool_manager($zfs_storage_id, $zpool) {
	...
	// zpool table
	$table = new htmlobject_table_builder('zpool_lun_name', '', '', '', 'luns');
	$arHead = array();
