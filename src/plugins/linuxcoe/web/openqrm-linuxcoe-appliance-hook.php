	...
	switch($cmd) {
		case "start":
			...
			if ($linuxcoe_auto_install_enabled) {
				...
				$linuxcoe_command = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/linuxcoe/bin/openqrm-linuxcoe-manager transfer_to_linuxcoe -x ".$resource->id." -i ".$resource_ip." -m ".$resource_mac." -n ".$linuxcoe_install_profile;
				...
				$openqrm_server->send_command($linuxcoe_command);
				...
				$linuxcoe_state_fields["linuxcoe_id"]=openqrm_db_get_free_id('linuxcoe_id', $linuxcoe_state->_db_table);
				...
				if (strcmp($image->type, "linuxcoe-deployment")) {
					...
					$linuxcoe_command = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/linuxcoe/bin/openqrm-linuxcoe-manager set_linuxcoe_client_to_local_boot -m ".$resource_mac;
					$openqrm_server->send_command($linuxcoe_command);
				}
			}
			break;
