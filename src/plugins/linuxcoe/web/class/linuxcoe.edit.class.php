...
var $lang = array();
	...
	function __construct($openqrm, $response) {
		...
		require_once($this->openqrm->get('webdir').'/plugins/linuxcoe/class/linuxcoe-volume.class.php');
	}
	...
	function edit() {
		if($this->deployment->type === 'linuxcoe-deployment') {
			...
			$table->sort            = 'linuxcoe_volume_name';
			$table->limit           = 10;
			$table->offset          = 0;
			$table->order           = 'ASC';
			...
			$linuxcoe_volume_arr = $linuxcoe_volume->display_overview($table->offset, $table->limit, $table->sort, $table->order);
			if(count($linuxcoe_volume_arr) >= 1) {
				foreach($linuxcoe_volume_arr as $k => $v) {
					...
					$body[] = array(
						'icon' => $d['icon'],
						'id' => $v['linuxcoe_volume_id'],
						'name'   => $v['linuxcoe_volume_name'],
						'root'   => $v['linuxcoe_volume_root'],
