			...
			case 'start':
				if (isset($_REQUEST['identifier_table1'])) {
					...
					foreach($_REQUEST['identifier_table1'] as $xen_name) {
						...
						// send command
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen/bin/openqrm-xen start -n $xen_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						$xen->send_command($xen->ip, $resource_command);
						// wait for statfile to appear again
						if (!wait_for_statfile($statfile, false)) {
			...
			case 'stop':
				if (isset($_REQUEST['identifier_table1'])) {
					...
					foreach($_REQUEST['identifier_table1'] as $xen_name) {
						...
						// send command
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen/bin/openqrm-xen stop -n $xen_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						$xen->send_command($xen->ip, $resource_command);
						// wait for statfile to appear again
						if (!wait_for_statfile($statfile, false)) {
			...
			case 'reboot':
				if (isset($_REQUEST['identifier_table1'])) {
					...
					foreach($_REQUEST['identifier_table1'] as $xen_name) {
						...
						// send command
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen/bin/openqrm-xen reboot -n $xen_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						$xen->send_command($xen->ip, $resource_command);
						// wait for statfile to appear again
						if (!wait_for_statfile($statfile, false)) {
			...
			case 'remove':
				if (isset($_REQUEST['identifier_table1'])) {
					...
					foreach($_REQUEST['identifier_table1'] as $xen_name) {
						...
						// send command
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen/bin/openqrm-xen remove -n $xen_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						$xen->send_command($xen->ip, $resource_command);
				...
				if (isset($_REQUEST['identifier_table1'])) {
					...
					foreach($_REQUEST['identifier_table1'] as $xen_name) {
						...
						$destination->get_instance_by_id($xen_migrate_to_id);
                                                ...
                                                $destination_command = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/xen/bin/openqrm-xen create_vm_config -n ".$xen_name;
						...
						if ($xen_migrate_type == "1") {
                                                ...
                                                $resource_command=$OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/xen/bin/openqrm-xen migrate -n ".$xen_name." -i ".$destination->ip." ".$xen_migrate_parameter." -u ".$OPENQRM_ADMIN->name." -p ".$OPENQRM_ADMIN->password;
						$xen->send_command($xen->ip, $resource_command);
						// wait for statfile to appear again
						if (!wait_for_statfile($statfile, true)) {
							$strMsg .= "Error while migrating Xen vm $xen_name to Host resource $xen_migrate_to_id! Please check the Event-Log<br>";
                                                        // remove config created on the destionation
                                                        $destination_command = $OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/xen/bin/openqrm-xen remove_vm_config -n ".$xen_name;
                                                        $destination->send_command($destination->ip, $destination_command);
							continue;
						} else {
							$strMsg .= "Migrated Xen vm $xen_name to Host resource $xen_migrate_to_id<br>";
							unlink($statfile);
						}
				...
				if (isset($_REQUEST['identifier_table1'])) {
					foreach($_REQUEST['identifier_table1'] as $xen_server_name) {
						...
						foreach ($enabled_plugins as $index => $plugin_name) {
							...
							if (file_exists($plugin_remote_console_hook)) {
								if (file_exists($plugin_remote_console_running)) {
									$event->log("console", $_SERVER['REQUEST_TIME'], 5, "xen-manager.php", "Found plugin $plugin_name providing a remote console.", "", "", 0, 0, $xen_resource->id);
									require_once "$plugin_remote_console_hook";
	...
	if (file_exists($xen_vm_list_file)) {
		...
		foreach ($xen_vm_list_content as $index => $xenxmoutput) {
			...
			if ($xen_vm_online == 1) {
				...
				if ($xen_openqrm_vm == 1) {
					$xen_vm_actions = $xen_vm_actions."<a href=\"xen-vm-config.php?xen_name=$xen_name&xen_id=$xen_appliance->id\" style=\"text-decoration:none;\"><img height=20 width=20 src=\"/openqrm/base/plugins/aa_plugins/img/plugin.png\" border=\"0\"> Config</a>&nbsp;";
