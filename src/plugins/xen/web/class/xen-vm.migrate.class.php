...
var $lang = array();
	...
	function migrate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$destination_command = $this->openqrm->get('basedir').'/plugins/xen/bin/openqrm-xen create_vm_config -n '.$this->vm;
			$destination->send_command($dest_host_resource->ip, $destination_command);
			// give some time
			sleep(4);
			// remove current stat file
			$statfile= $this->openqrm->get('webdir')."/plugins/xen/xen-stat/".$this->vm.".vm_migrated_successfully";
			if (file_exists($statfile)) {
				unlink($statfile);
			}
			// send command
			$xen_migrate_parameter = "-t live";
			$resource_command = $this->openqrm->get('basedir')."/plugins/xen/bin/openqrm-xen migrate -n ".$this->vm." -i ".$destination->ip." ".$xen_migrate_parameter." -u ".$this->openqrm->admin()->name." -p ".$this->openqrm->admin()->password;
