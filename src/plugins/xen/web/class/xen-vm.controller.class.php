...
var $lang = array(
	...
	function select( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller = new xen_vm_select($this->openqrm, $this->response);
	...
	function edit( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload()) {
				...
				$controller                  = new xen_vm_edit($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
	...
	function add( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload() && $this->reload_bridges()) {
				...
				$controller                = new xen_vm_add($this->openqrm, $this->response, $this);
				$controller->actions_name  = $this->actions_name;
				$controller->tpldir        = $this->tpldir;
				$controller->message_param = $this->message_param;
	...
	function update( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload() && $this->reload_bridges()) {
				...
				$controller                = new xen_vm_update($this->openqrm, $this->response);
				$controller->actions_name  = $this->actions_name;
				$controller->tpldir        = $this->tpldir;
				$controller->message_param = $this->message_param;
	...
	function console( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload()) {
				...
				$controller                  = new xen_vm_console($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function duplicate( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload()) {
				...
				$controller                  = new xen_vm_clone($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function migrate( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new xen_vm_migrate($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function remove( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new xen_vm_remove($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function start( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new xen_vm_start($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function stop( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new xen_vm_stop($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function reboot( $hidden = true ) {
		...
		if( $hidden === true ) {
			...
			$controller                  = new xen_vm_reboot($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function reload() {
		...
		$command  = $OPENQRM_SERVER_BASE_DIR.'/plugins/xen/bin/openqrm-xen post_vm_list';
		...
		$file = $OPENQRM_SERVER_BASE_DIR.'/plugins/xen/web/xen-stat/'.$resource->id.'.vm_list';
	...
	function reload_bridges() {
		...
		$OPENQRM_SERVER_BASE_DIR = $this->openqrm->get('basedir');
		$command  = $OPENQRM_SERVER_BASE_DIR.'/plugins/xen/bin/openqrm-xen post_bridge_config';
		$command .= ' -u '.$this->openqrm->admin()->name.' -p '.$this->openqrm->admin()->password;
		$id = $this->response->html->request()->get('appliance_id');
		$appliance = new appliance();
		$appliance->get_instance_by_id($id);
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$file = $OPENQRM_SERVER_BASE_DIR.'/plugins/xen/web/xen-stat/'.$resource->id.'.bridge_config';
		if($this->file->exists($file)) {
			$this->file->remove($file);
		}
		$resource->send_command($resource->ip, $command);
