								...
								if($line !== '') {
									...
									if($vm === $line[1]) {
										...
										if (count($appliances_using_resource) > 0) {
											...
											$command  = $this->openqrm->get('basedir').'/plugins/xen/bin/openqrm-xen remove -n '.$vm;
											...
											foreach ($enabled as $index => $name) {
												...
												if (file_exists($hook)) {
													if (file_exists($running)) {
														$event->log("console", $_SERVER['REQUEST_TIME'], 5, "xen-vm.remove.class.php", 'Found plugin '.$name.' providing a remote console.', "", "", 0, 0, $xenvm->id);
														require_once($hook);
