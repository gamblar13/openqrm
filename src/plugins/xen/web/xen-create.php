...
if(htmlobject_request('xen_command') != '') {
	$event->log("action", $_SERVER['REQUEST_TIME'], 5, "xen-create", "Processing create command", "", "", 0, 0, 0);
	if ($OPENQRM_USER->role == "administrator") {
		...
		switch ($xen_command) {
			case 'new':
				...
				sleep(5);
				// send command
				$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/xen/bin/openqrm-xen create -n $xen_name -m $xen_mac -r $xen_ram -c $xen_cpus $xen_vm_disk_param $xen_vm_swap_param -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
