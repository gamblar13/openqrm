			...
			case 'start':
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $vbox_server_name) {
						...
						$vbox_server = new resource();
						$vbox_server->get_instance_by_id($vbox_appliance->resources);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vbox/bin/openqrm-vbox start -n $vbox_server_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$vbox_server_resource_id = $vbox_server->id;
						$statfile="vbox-stat/".$vbox_server_resource_id.".vm_list";
						if (file_exists($statfile)) {
			...
			case 'stop':
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $vbox_server_name) {
						...
						$vbox_server = new resource();
						$vbox_server->get_instance_by_id($vbox_appliance->resources);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vbox/bin/openqrm-vbox stop -n $vbox_server_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$vbox_server_resource_id = $vbox_server->id;
						$statfile="vbox-stat/".$vbox_server_resource_id.".vm_list";
						if (file_exists($statfile)) {
			...
			case 'restart':
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $vbox_server_name) {
						...
						$vbox_server = new resource();
						$vbox_server->get_instance_by_id($vbox_appliance->resources);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vbox/bin/openqrm-vbox reboot -n $vbox_server_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$vbox_server_resource_id = $vbox_server->id;
						$statfile="vbox-stat/".$vbox_server_resource_id.".vm_list";
						if (file_exists($statfile)) {
			...
			case 'delete':
				if (isset($_REQUEST['identifier'])) {
					foreach($_REQUEST['identifier'] as $vbox_server_name) {
						...
						$vbox_server = new resource();
						$vbox_server->get_instance_by_id($vbox_appliance->resources);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vbox/bin/openqrm-vbox delete -n $vbox_server_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$vbox_server_resource_id = $vbox_server->id;
						$statfile="vbox-stat/".$vbox_server_resource_id.".vm_list";
						if (file_exists($statfile)) {
