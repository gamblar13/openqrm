	...
	if ($OPENQRM_USER->role == "administrator") {
		...
		switch (htmlobject_request('action')) {
			case 'update_cpus':
					...
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vbox/bin/openqrm-vbox update_vm_cpus -n $vbox_server_name -c $vbox_update_cpus -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
			...
			case 'update_ram':
					...
					$vbox_server = new resource();
					$vbox_server->get_instance_by_id($vbox_server_appliance->resources);
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vbox/bin/openqrm-vbox update_vm_ram -n $vbox_server_name -r $vbox_update_ram -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$vbox_server_resource_id = $vbox_server->id;
					$statfile="vbox-stat/".$vbox_server_resource_id.".".$vbox_server_name.".vm_config";
					if (file_exists($statfile)) {
			...
			case 'add_vm_net':
					...
					$vbox_server = new resource();
					$vbox_server->get_instance_by_id($vbox_server_appliance->resources);
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vbox/bin/openqrm-vbox add_vm_nic -n $vbox_server_name -x $vbox_nic_nr -m $vbox_new_nic -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$vbox_server_resource_id = $vbox_server->id;
					$statfile="vbox-stat/".$vbox_server_resource_id.".".$vbox_server_name.".vm_config";
					if (file_exists($statfile)) {
			...
			case 'remove_vm_net':
					...
					$vbox_server = new resource();
					$vbox_server->get_instance_by_id($vbox_server_appliance->resources);
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vbox/bin/openqrm-vbox remove_vm_nic -n $vbox_server_name -x $vbox_nic_nr -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$vbox_server_resource_id = $vbox_server->id;
					$statfile="vbox-stat/".$vbox_server_resource_id.".".$vbox_server_name.".vm_config";
					if (file_exists($statfile)) {
			...
			case 'add_vm_disk':
					...
					$vbox_server = new resource();
					$vbox_server->get_instance_by_id($vbox_server_appliance->resources);
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vbox/bin/openqrm-vbox add_vm_disk -n $vbox_server_name -x $vbox_disk_nr -d $vbox_new_disk -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$vbox_server_resource_id = $vbox_server->id;
					$statfile="vbox-stat/".$vbox_server_resource_id.".".$vbox_server_name.".vm_config";
					if (file_exists($statfile)) {
			...
			case 'remove_vm_disk':
					...
					$vbox_server = new resource();
					$vbox_server->get_instance_by_id($vbox_server_appliance->resources);
					$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/vbox/bin/openqrm-vbox remove_vm_disk -n $vbox_server_name -x $vbox_disk_nr -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					// remove current stat file
					$vbox_server_resource_id = $vbox_server->id;
					$statfile="vbox-stat/".$vbox_server_resource_id.".".$vbox_server_name.".vm_config";
					if (file_exists($statfile)) {
...
function vbox_vm_config() {
	...
	if (isset($store['OPENQRM_VBOX_VM_MAC1'])) {
	...
	if (isset($store['OPENQRM_VBOX_VM_MAC2'])) {
	...
	if (isset($store['OPENQRM_VBOX_VM_MAC3'])) {
	...
	if (isset($store['OPENQRM_VBOX_VM_MAC4'])) {
...
function vbox_vm_config_disk() {
	...
	$disk_count=1;
	if (isset($store['OPENQRM_VBOX_VM_DISK_SIZE_1'])) {
		if (strlen($store['OPENQRM_VBOX_VM_DISK_SIZE_1'])) {
	...
	if (isset($store['OPENQRM_VBOX_VM_DISK_SIZE_2'])) {
		if (strlen($store['OPENQRM_VBOX_VM_DISK_SIZE_2'])) {
	...
	if (isset($store['OPENQRM_VBOX_VM_DISK_SIZE_3'])) {
		if (strlen($store['OPENQRM_VBOX_VM_DISK_SIZE_3'])) {
