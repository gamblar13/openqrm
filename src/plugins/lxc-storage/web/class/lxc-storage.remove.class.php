...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/lxc-storage-remove.tpl.php');
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($lvols as $key => $lvol) {
					...
					if (count($appliances_using_resource) > 0) {
						...
						$command  = $this->openqrm->get('basedir').'/plugins/lxc-storage/bin/openqrm-lxc-storage remove';
						$command .= ' -n '.$lvol;
						$command .= ' -v '.$this->volgroup;
						$command .= ' -u '.$name.' -p '.$pass;

						$file = $this->openqrm->get('basedir').'/plugins/lxc-storage/web/storage/'.$resource->id.'.'.$this->volgroup.'.lv.stat';
