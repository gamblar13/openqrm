...
var $lang = array();
	...
	function init() {
		...
		$this->statfile = $this->openqrm->get('basedir').'/plugins/lxc-storage/web/storage/'.$resource->id.'.'.$this->volgroup.'.lv.stat';
	}
	...
	function action() {
		...
		$data = $this->edit();
		if($data !== false) {
			$t = $this->response->html->template($this->tpldir.'/lxc-storage-volgroup.tpl.php');
	...
	function edit() {
		if(strpos($this->deployment->type, 'lxc-lvm-deployment') !== false) {
			...
			// Volgroup info
			$lines = explode("\n", $this->file->get_contents($this->openqrm->get('basedir').'/plugins/lxc-storage/web/storage/'.$this->resource->id.'.vg.stat'));
					...
					foreach($lines as $line) {
						if($line !== '') {
							...
							if ($line[4] == "clone_in_progress") {
								...
								// progressbar
								$t->add(uniqid('b'), 'id');
								$t->add('/openqrm/base/plugins/lxc-storage/storage/'.$this->resource->id.'.lvm.'.$name.'.sync_progress', 'url');
