...
class lxc_storage_vm_controller
{
/**
...
* @var string
*/
var $identifier_name = 'lxc_storage_vm_identifier';
/**
* path to templates
* @access public
* @var string
...
var $lang = array(
	'select' => array (
		'tab' => 'Select LXC Host',
		'label' => 'Select LXC-Storage Host Appliance',
		'action_edit' => 'edit',
		'table_name' => 'Name',
		'table_id' => 'Id',
		...
		'error_no_host' => '<b>No LXC-Storage Host Appliance configured yet!</b><br><br>Please create a LXC-Storage Host Appliance first!',
		'new' => 'New Appliance',
		'please_wait' => 'Loading VMs. Please wait ..',
	), 
	'edit' => array (
		'tab' => 'Edit LXC Host',
		'label' => 'LXC VMs on LXC-Storage Host Appliance %s',
		'lang_id' => 'ID',
		'lang_name' => 'Name',
	...
	'migrate' => array (
		...
		'error_no_hosts' => 'No LXC Host found to migrate to',
	...
	function __construct($openqrm, $response) {
		...
		$this->lang     = $this->user->translate($this->lang, $this->rootdir."/plugins/lxc-storage/lang", 'lxc-storage-vm.ini');
	...
	function select( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/lxc-storage/class/lxc-storage-vm.select.class.php');
			$controller = new lxc_storage_vm_select($this->openqrm, $this->response);
	...
	function edit( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload()) {
				require_once($this->rootdir.'/plugins/lxc-storage/class/lxc-storage-vm.edit.class.php');
				$controller                  = new lxc_storage_vm_edit($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
	...
	function add( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload() && $this->reload_bridges()) {
				require_once($this->rootdir.'/plugins/lxc-storage/class/lxc-storage-vm.add.class.php');
				$controller                = new lxc_storage_vm_add($this->openqrm, $this->response, $this);
				$controller->actions_name  = $this->actions_name;
				$controller->tpldir        = $this->tpldir;
				$controller->message_param = $this->message_param;
	...
	function update( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload() && $this->reload_bridges()) {
				require_once($this->rootdir.'/plugins/lxc-storage/class/lxc-storage-vm.update.class.php');
				$controller                = new lxc_storage_vm_update($this->openqrm, $this->response);
				$controller->actions_name  = $this->actions_name;
				$controller->tpldir        = $this->tpldir;
				$controller->message_param = $this->message_param;
	...
	function duplicate( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload()) {
				require_once($this->rootdir.'/plugins/lxc-storage/class/lxc-storage-vm.clone.class.php');
				$controller                  = new lxc_storage_vm_clone($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function migrate( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/lxc-storage/class/lxc-storage-vm.migrate.class.php');
			$controller                  = new lxc_storage_vm_migrate($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function remove( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/lxc-storage/class/lxc-storage-vm.remove.class.php');
			$controller                  = new lxc_storage_vm_remove($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function start( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/lxc-storage/class/lxc-storage-vm.start.class.php');
			$controller                  = new lxc_storage_vm_start($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function stop( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/lxc-storage/class/lxc-storage-vm.stop.class.php');
			$controller                  = new lxc_storage_vm_stop($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function reboot( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/lxc-storage/class/lxc-storage-vm.reboot.class.php');
			$controller                  = new lxc_storage_vm_reboot($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function reload() {
		...
		$command  = $OPENQRM_SERVER_BASE_DIR.'/plugins/lxc-storage/bin/openqrm-lxc-storage-vm post_vm_list';
		...
		$file = $OPENQRM_SERVER_BASE_DIR.'/plugins/lxc-storage/web/lxc-stat/'.$resource->id.'.vm_list';
	...
	function reload_bridges() {
		$OPENQRM_SERVER_BASE_DIR = $this->openqrm->get('basedir');
		$command  = $OPENQRM_SERVER_BASE_DIR.'/plugins/lxc-storage/bin/openqrm-lxc-storage-vm post_bridge_config';
		$command .= ' -u '.$this->openqrm->admin()->name.' -p '.$this->openqrm->admin()->password;
		$id = $this->response->html->request()->get('appliance_id');
		$appliance = new appliance();
		$appliance->get_instance_by_id($id);
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$file = $OPENQRM_SERVER_BASE_DIR.'/plugins/lxc-storage/web/lxc-stat/'.$resource->id.'.bridge_config';
		if($this->file->exists($file)) {
			$this->file->remove($file);
		}
		$resource->send_command($resource->ip, $command);
