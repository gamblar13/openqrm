...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/lxc-storage-clone.tpl.php');
	...
	function duplicate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$command  = $this->openqrm->get('basedir').'/plugins/lxc-storage/bin/openqrm-lxc-storage clone';
			$command .= ' -v '.$this->volgroup;
			$command .= ' -n '.$this->lvol;
			$command .= ' -s '.$name;
			$command .= ' -u '.$this->openqrm->admin()->name.' -p '.$this->openqrm->admin()->password;

			$statfile = $this->openqrm->get('basedir').'/plugins/lxc-storage/web/storage/'.$this->resource->id.'.'.$this->volgroup.'.lv.stat';
			...
			if(isset($error)) {
				...
				$file = $this->openqrm->get('basedir').'/plugins/lxc-storage/web/storage/'.$this->resource->id.'.lvm.'.$name.'.sync_progress';
				if($this->file->exists($file)) {
					$this->file->remove($file);
				}
				$root_device_ident = $this->openqrm->get('basedir').'/plugins/lxc-storage/web/storage/'.$this->resource->id.'.'.$name.'.root_device';
			...
			if(count($lines) >= 1) {
				foreach($lines as $line) {
					if($line !== '') {
						$line = explode('@', $line);
						if($line[1] === $this->lvol) {
							$lxcax = str_replace('.', '', $line[4]);
							$lxcax = str_replace('m', '', $lxcax);
							$lxcax = (int)$lxcax / 100;
						}
					}
				}
