...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/lxc-storage-vm-add.tpl.php');
	...
	function add() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(count($errors) > 0 || $form->get_errors()) {
				...
				$command  = $this->openqrm->get('basedir').'/plugins/lxc-storage/bin/openqrm-lxc-storage-vm create';
				$command .= ' -n '.$name;
				$command .= ' -m '.$mac;
	...
	function get_response() {
		...
		$file = $OPENQRM_SERVER_BASE_DIR.'/plugins/lxc-storage/web/lxc-stat/'.$this->resource->id.'.bridge_config';
