...
var $lang = array();
	...
	function action() {
		$table = $this->select();
		$t = $this->response->html->template($this->tpldir.'/lxc-storage-select.tpl.php');
	...
	function select() {
		...
		if(count($storages) >= 1) {
			foreach($storages as $k => $v) {
				...
				if($deployment->storagetype === 'lxc-storage') {
			...
			$table = $this->response->html->tablebuilder('lxc', $this->response->get_array($this->actions_name, 'select'));
