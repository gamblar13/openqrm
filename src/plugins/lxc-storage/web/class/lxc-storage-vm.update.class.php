...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/lxc-storage-vm-update.tpl.php');
	...
	function update() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(count($errors) > 0 || $form->get_errors()) {
				...
				$command  = $this->openqrm->get('basedir').'/plugins/lxc-storage/bin/openqrm-lxc-storage-vm update';
				$command .= ' -n '.$this->vm;
	...
	function get_response() {
		...
		$file = $this->openqrm->get('basedir').'/plugins/lxc-storage/web/lxc-stat/'.$this->resource->id.'.'.$this->vm.'.vm_config';
		...
		$this->response->params['mac'] = $data['OPENQRM_LXC_MAC'];
		...
		$d['mac']['object']['attrib']['value']     = $data['OPENQRM_LXC_MAC'];
		...
		$d['bridge']['object']['attrib']['selected'] = array($data['OPENQRM_LXC_BRIDGE']);

		// net 1
		$checked = false;
		if($data['OPENQRM_LXC_MAC2'] !== '' ) {
			$mac = $data['OPENQRM_LXC_MAC2'];
			$checked = true;
		} else {
		...
		if(isset($data['OPENQRM_LXC_BRIDGE_NET2']) && $checked === true) {
			$d['bridge1']['object']['attrib']['selected'] = array($data['OPENQRM_LXC_BRIDGE_NET2']);
		}

		// net 2
		$checked = false;
		if($data['OPENQRM_LXC_MAC3'] !== '' ) {
			$mac = $data['OPENQRM_LXC_MAC3'];
			$checked = true;
		} else {
			$this->resource->generate_mac();
			$mac = $this->resource->mac;
		...
		if(isset($data['OPENQRM_LXC_BRIDGE_NET3']) && $checked === true) {
			$d['bridge2']['object']['attrib']['selected'] = array($data['OPENQRM_LXC_BRIDGE_NET3']);
		}

		// net 3
		$checked = false;
		if($data['OPENQRM_LXC_MAC4'] !== '' ) {
			$mac = $data['OPENQRM_LXC_MAC4'];
			$checked = true;
		} else {
			$this->resource->generate_mac();
			$mac = $this->resource->mac;
		...
		if(isset($data['OPENQRM_LXC_BRIDGE_NET4']) && $checked === true) {
			$d['bridge3']['object']['attrib']['selected'] = array($data['OPENQRM_LXC_BRIDGE_NET4']);
		}

		// net 4
		$checked = false;
		if($data['OPENQRM_LXC_MAC5'] !== '' ) {
			$mac = $data['OPENQRM_LXC_MAC5'];
			$checked = true;
		} else {
			$this->resource->generate_mac();
			$mac = $this->resource->mac;
		...
		if(isset($data['OPENQRM_LXC_BRIDGE_NET5']) && $checked === true) {
			$d['bridge4']['object']['attrib']['selected'] = array($data['OPENQRM_LXC_BRIDGE_NET5']);
		}
	...
	//--------------------------------------------
	function reload() {
		$command  = $this->openqrm->get('basedir').'/plugins/lxc-storage/bin/openqrm-lxc-storage-vm post_vm_config';
		$command .=  ' -n '.$this->vm;
		$command .= ' -u '.$this->openqrm->admin()->name.' -p '.$this->openqrm->admin()->password;
		$id = $this->response->html->request()->get('appliance_id');
		$appliance = new appliance();
		...
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$file = $this->openqrm->get('basedir').'/plugins/lxc-storage/web/lxc-stat/'.$resource->id.'.'.$this->vm.'.vm_config';
