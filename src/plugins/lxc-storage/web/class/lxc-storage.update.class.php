...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/lxc-storage-update.tpl.php');
	...
	function update() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$this->lxctemplate->get_instance_by_name($template);
			if ($this->lxctemplate->id === '') {
				// add
				$lxc_fields["lxc_template_id"] = openqrm_db_get_free_id('lxc_template_id', $this->lxctemplate->_db_table);
				$lxc_fields["lxc_template_name"] = $template;
				$lxc_fields["lxc_template_description"] = $comment;
				$this->lxctemplate->add($lxc_fields);
			} else {
				// update
				$lxc_fields["lxc_template_description"] = $comment;
				$this->lxctemplate->update($this->lxctemplate->id, $lxc_fields);
			}
			$this->lxctemplate->id = '';
