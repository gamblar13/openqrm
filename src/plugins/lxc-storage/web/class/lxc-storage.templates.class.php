...
var $lang = array();
	...
	function init() {
		...
		$this->statfile = $this->openqrm->get('basedir').'/plugins/lxc-storage/web/storage/lxc-templates.stat';
	...
	function action() {
		...
		$data = $this->templates();
		if($data !== false) {
			$t = $this->response->html->template($this->tpldir.'/lxc-storage-templates.tpl.php');
			...
			$msg = sprintf($this->lang['error_no_lxc'], $this->response->html->request()->get('storage_id'));
	...
	//--------------------------------------------
	function templates() {
		if(strpos($this->deployment->type, 'lxc-lvm-deployment') !== false) {
				...
				if(count($lines) >= 1) {
					foreach($lines as $line) {
						if($line !== '') {
							...
							// description
							$comment = ' ';
							$this->lxctemplate->get_instance_by_name($name);
							if ($this->lxctemplate->id === '') {
								// add
								$lxc_fields["lxc_template_id"] = openqrm_db_get_free_id('lxc_template_id', $this->lxctemplate->_db_table);
								$lxc_fields["lxc_template_name"] = $name;
								$this->lxctemplate->add($lxc_fields);
							} else {
								if (strlen($this->lxctemplate->description)) {
									$comment = $this->lxctemplate->description;
								}
							}
							$this->lxctemplate->id = '';
