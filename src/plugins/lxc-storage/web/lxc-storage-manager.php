        ...
        switch (htmlobject_request('action')) {
            ...
            case 'select':
                if (isset($_REQUEST['identifier'])) {
                    foreach($_REQUEST['identifier'] as $id) {
                        ...
                        // post vg status
                        $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage post_vg -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
            ...
            case 'select-vg':
                if (isset($_REQUEST['identifier'])) {
                    foreach($_REQUEST['identifier'] as $lvm_volume_group) {
                        ...
                        $storage_resource->get_instance_by_id($storage->resource_id);
                        // post lv status
                        $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage post_lv -u $OPENQRM_USER->name -p $OPENQRM_USER->password -v $lvm_volume_group";
                        // remove current stat file
                        $storage_resource_id = $storage_resource->id;
            ...
            case 'add':
                ...
                if (!strcmp($storage_deployment->type, "lvm-iscsi-deployment")) {
                    ...
                    $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage add -n $lvm_lun_name -v $lvm_volume_group -m $lvm_lun_size -i $image_password -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                } else {
                    $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage add -n $lvm_lun_name -v $lvm_volume_group -m $lvm_lun_size -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                }
                // remove current stat file
                $storage_resource_id = $storage_resource->id;
                $statfile="storage/".$storage_resource_id.".".$lvm_volume_group.".lv.stat";
            ...
            case 'remove':
                if (isset($_REQUEST['identifier'])) {
                    ...
                    foreach($_REQUEST['identifier'] as $lvm_lun_name) {
                        ...
                        $storage_deployment = new deployment();
                        $storage_deployment->get_instance_by_id($storage->type);
                        $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage remove -n $lvm_lun_name -v $lvm_volume_group -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                        // remove current stat file
                        $storage_resource_id = $storage_resource->id;
                        $statfile="storage/".$storage_resource_id.".".$lvm_volume_group.".lv.stat";
                        if (file_exists($statfile)) {
            ...
            case 'clone':
                ...
                if (strlen($lvm_lun_snap_name)) {
                    ...
                    if (!strcmp($deployment->type, "lvm-iscsi-deployment")) {
                        ...
                        // generate a password for the image
                        $image_password = $image->generatePassword(12);
                        $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage $lvm_action -n $lvm_lun_name -v $lvm_volume_group -s $lvm_lun_snap_name -m $lvm_lun_snap_size -i $image_password -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                    } else {
                        $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage $lvm_action -n $lvm_lun_name -v $lvm_volume_group -s $lvm_lun_snap_name -m $lvm_lun_snap_size -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                    }
                    // remove current stat file
                    $storage_resource_id = $storage_resource->id;
                    $statfile="storage/".$storage_resource_id.".".$lvm_volume_group.".lv.stat";
            ...
            case 'resize':
                ...
                if (!strcmp($deployment->type, "lvm-iscsi-deployment")) {
                    ...
                    // generate a password for the image
                    $image_password = $image->generatePassword(12);
                    $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage resize -n $lvm_lun_name -v $lvm_volume_group -m $lvm_lun_resize -i $image_password -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                } else {
                    $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage resize -n $lvm_lun_name -v $lvm_volume_group -m $lvm_lun_resize -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
                }
                // remove current stat file
                $storage_resource_id = $storage_resource->id;
                $statfile="storage/".$storage_resource_id.".".$lvm_volume_group.".lv.stat";
