...
// to gather one of the deployment types within lxc-storage
...
function redirect_deploy($strMsg, $lvm_storage_id, $lvm_volume_group, $lvm_lun_name) {
	global $thisfile;
	global $action;
	$url = 'lxc-storage-deploy.php?strMsg='.urlencode($strMsg).'&currenttab=tab0&redirect=yes&action='.$action.'&lvm_storage_id='.$lvm_storage_id.'&lvm_volume_group='.$lvm_volume_group.'&lvm_lun_name='.$lvm_lun_name;
	echo "<meta http-equiv=\"refresh\" content=\"0; URL=$url\">";
	exit;
}
			...
			switch (htmlobject_request('action')) {
				...
				case 'select':
					if (isset($_REQUEST['identifier'])) {
						foreach($_REQUEST['identifier'] as $id) {
							...
							// post vg status
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage post_vg -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
				...
				case 'select-vg':
					if (isset($_REQUEST['identifier'])) {
						foreach($_REQUEST['identifier'] as $lvm_volume_group) {
							...
							$storage_resource->get_instance_by_id($storage->resource_id);
							// post lv status
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage post_lv -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password -v $lvm_volume_group";
							// remove current stat file
							$storage_resource_id = $storage_resource->id;
				...
				case 'add':
					...
					if (!strcmp($storage_deployment->type, "lvm-iscsi-deployment")) {
						...
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage add -n $lvm_lun_name -v $lvm_volume_group -m $lvm_lun_size -i $image_password -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					} else {
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage add -n $lvm_lun_name -v $lvm_volume_group -m $lvm_lun_size -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					}
					// remove current stat file
					$storage_resource_id = $storage_resource->id;
					$statfile="storage/".$storage_resource_id.".".$lvm_volume_group.".lv.stat";
				...
				case 'remove':
					if (isset($_REQUEST['identifier'])) {
						...
						foreach($_REQUEST['identifier'] as $lvm_lun_name) {
							...
							$storage_deployment = new deployment();
							$storage_deployment->get_instance_by_id($storage->type);
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage remove -n $lvm_lun_name -v $lvm_volume_group -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
							// remove current stat file
							$storage_resource_id = $storage_resource->id;
							$statfile="storage/".$storage_resource_id.".".$lvm_volume_group.".lv.stat";
							if (file_exists($statfile)) {
				...
				case 'clone':
					...
					if (strlen($lvm_lun_snap_name)) {
						...
						if (!strcmp($deployment->type, "lvm-iscsi-deployment")) {
							...
							// generate a password for the image
							$image_password = $image->generatePassword(12);
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage $lvm_action -n $lvm_lun_name -v $lvm_volume_group -s $lvm_lun_snap_name -m $lvm_lun_snap_size -i $image_password -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						} else {
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage $lvm_action -n $lvm_lun_name -v $lvm_volume_group -s $lvm_lun_snap_name -m $lvm_lun_snap_size -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						}
						// remove current stat file
						$storage_resource_id = $storage_resource->id;
						$statfile="storage/".$storage_resource_id.".".$lvm_volume_group.".lv.stat";
				...
				case 'resize':
					...
					if (!strcmp($deployment->type, "lvm-iscsi-deployment")) {
						...
						// generate a password for the image
						$image_password = $image->generatePassword(12);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage resize -n $lvm_lun_name -v $lvm_volume_group -m $lvm_lun_resize -i $image_password -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					} else {
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage resize -n $lvm_lun_name -v $lvm_volume_group -m $lvm_lun_resize -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					}
					// remove current stat file
					$storage_resource_id = $storage_resource->id;
					$statfile="storage/".$storage_resource_id.".".$lvm_volume_group.".lv.stat";
...
function lvm_select_storage() {
	...
	$t_deployment->get_instance_by_type("lxc-lvm-deployment");
   ...
   // set template
	...
	$t->debug = false;
	$t->setFile('tplfile', './tpl/' . 'lxc-storage-select.tpl.php');
	$t->setVar(array(
		'formaction' => $thisfile,
...
function lvm_storage_display($lvm_storage_id) {
	...
	$t = new Template_PHPLIB();
	$t->debug = false;
	$t->setFile('tplfile', './tpl/' . 'lxc-storage-vgs.tpl.php');
	$t->setVar(array(
		'formaction' => $thisfile,
		'storage_name' => $storage->name,
...
function lvm_storage_lv_display($lvm_storage_id, $lvm_volume_group) {
	...
	$t = new Template_PHPLIB();
	$t->debug = false;
	$t->setFile('tplfile', './tpl/' . 'lxc-storage-luns.tpl.php');
	$t->setVar(array(
		'formaction' => $thisfile,
		'storage_name' => $storage->name,
