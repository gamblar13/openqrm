...
if(htmlobject_request('action') != '') {
	switch (htmlobject_request('action')) {
		case 'Add':
				...
				$resource_command=$OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage-vm addnic -n ".$lxc_server_name." -m".$new_lxc_nic_number." $lxc_new_nic_mac $lxc_storage_vm_bridge_str -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
		...
		case 'Remove':
				...
				$lxc_storage_resource = new resource();
				$lxc_storage_resource->get_instance_by_id($lxc_storage_appliance->resources);
				$resource_command=$OPENQRM_SERVER_BASE_DIR."/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage-vm removenic -n ".$lxc_server_name." -u ".$OPENQRM_USER->name." -p ".$OPENQRM_USER->password;
                // remove current stat file
                $lxc_storage_resource_id = $lxc_storage_resource->id;
                $statfile="lxc-stat/".$lxc_storage_resource_id.".".$lxc_server_name.".vm_net_config";
                if (file_exists($statfile)) {
    ...
    $resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/lxc-storage/bin/openqrm-lxc-storage-vm post_vm_net_config -n $lxc_server_name -u $OPENQRM_USER->name -p $OPENQRM_USER->password";
    // remove current stat file
...
function lxc_vm_net_config() {
    ...
    $lxc_bridge_select = htmlobject_select('lxc_storage_vm_bridge', $bridge_identifier_array, 'Network-Bridge', 0);
