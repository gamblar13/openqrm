...
function find_virtualization_host($v_plugin_name) {
	global $event;
...
function openqrm_highavailability_ha_hook($resource_id) {
	...
	if ($appliance->resources == $resource_id) {
        ...
        if ($appliance_virtualization == 1) {
            ...
            $virtualization_host_resource_id = find_virtualization_host($virtualization_plugin_name);
            if ($virtualization_host_resource_id < 0) {
                // we did not find any online virtualization host
        		$event->log("openqrm_ha_hook", $_SERVER['REQUEST_TIME'], 5, "openqrm-highavailability-ha-hook.php", "There is no active Virtualization hosts available!", "", "", 0, 0, 0);
                // TODO : try find an stopped virtualization host with a powerd-off resource, then wake up
                return;
            }
