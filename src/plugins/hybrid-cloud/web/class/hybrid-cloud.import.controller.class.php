<?php
/**
 * hybrid_cloud_import Controller
 *
...
class hybrid_cloud_import_controller
{
/**
...
* @var string
*/
var $prefix_tab = 'hybrid_cloud_import_tab';
/**
* identifier name
* @access public
* @var string
...
var $tpldir;
	...
	function action($action = null) {
		...
		if($this->action === 'target') {
			$this->response->add('instance_id', $this->response->html->request()->get('instance_id'));
		}
	...
	function select( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/hybrid-cloud/class/hybrid-cloud.import.select.class.php');
			$controller = new hybrid_cloud_import_select($this->openqrm, $this->response, $this->controller);
	...
	function target( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/hybrid-cloud/class/hybrid-cloud.import.target.class.php');
			$controller = new hybrid_cloud_import_target($this->openqrm, $this->response, $this->controller);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
