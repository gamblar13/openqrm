...
* @var string
*/
var $actions_name = 'aoe-storage-action';
/**
* message param
* @access public
* @var string
*/
var $message_param = "msg-aoe-storage";
/**
* openqrm rootdir
* @access public
* @var string
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/aoe-storage-add.tpl.php');
	...
	function add() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(!$form->get_errors()) {			
				...
				$command     = $this->openqrm->get('basedir')."/plugins/aoe-storage/bin/openqrm-aoe-storage add -n ".$name." -m ".$size;
					...
					if(count($lines) >= 1) {
						foreach($lines as $line) {
							if($line !== '') {
								...
								if($name === $check) {
									$created = true;
									$volume_path = $name.':/dev/etherd/e'.$line[1].'.'.$line[2];
									break;
								}
							}
						}
	...
	//--------------------------------------------
	function set_max() {
		$statfile = $this->openqrm->get('basedir').'/plugins/aoe-storage/web/storage/'.$this->resource->id.'.aoe.stat';
		if (file_exists($statfile)) {
			$lines = explode("\n", file_get_contents($statfile));
			if(count($lines) >= 1) {
				foreach($lines as $line) {
