...
var $lang = array(
	'select' => array (
		'tab' => 'Select AOE-storage',
		'label' => 'Select AOE-storage',
		'action_edit' => 'edit',
		'table_name' => 'Name',
		'table_id' => 'Id',
		...
		'error_no_storage' => '<b>No storage configured yet!</b><br><br>Please create a AOE Storage first!',
		'new_storage' => 'New Storage',
		'please_wait' => 'Loading Storage. Please wait ..',
	), 
	'edit' => array (
		'tab' => 'Edit AOE-storage',
		'label' => 'AOE Volumes on storage %s',
		'lang_id' => 'ID',
		'lang_name' => 'Name',
		...
		'table_shelf' => 'Shelf',
		'table_slot' => 'Slot',
		'table_mac' => 'Permission',
		'error_no_aoe' => 'Storage %s is not of type aoe-deployment',
	...
	function __construct($openqrm, $response) {
		...
		$this->lang     = $this->user->translate($this->lang, $this->rootdir."/plugins/aoe-storage/lang", 'aoe-storage.ini');
	...
	//--------------------------------------------
	function api() {
		require_once($this->rootdir.'/plugins/aoe-storage/class/aoe-storage.api.class.php');
		$controller = new aoe_storage_api($this);
		$controller->action();
	}
	...
	function select( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/aoe-storage/class/aoe-storage.select.class.php');
			$controller = new aoe_storage_select($this->openqrm, $this->response);
	...
	function edit( $hidden = true ) {
		...
		if( $hidden === true ) {
			if($this->reload()) {
				require_once($this->rootdir.'/plugins/aoe-storage/class/aoe-storage.edit.class.php');
				$controller                  = new aoe_storage_edit($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
	...
	function add( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/aoe-storage/class/aoe-storage.add.class.php');
			$controller                = new aoe_storage_add($this->openqrm, $this->response, $this);
			$controller->actions_name  = $this->actions_name;
			$controller->tpldir        = $this->tpldir;
			$controller->message_param = $this->message_param;
	...
	function remove( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/aoe-storage/class/aoe-storage.remove.class.php');
			$controller                  = new aoe_storage_remove($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function duplicate( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/aoe-storage/class/aoe-storage.clone.class.php');
			$controller                  = new aoe_storage_clone($this->openqrm, $this->response, $this);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function auth( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/aoe-storage/class/aoe-storage.auth.class.php');
			$controller                  = new aoe_storage_auth($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function reload() {
		...
		$command  = $OPENQRM_SERVER_BASE_DIR."/plugins/aoe-storage/bin/openqrm-aoe-storage post_luns";
		...
		$file = $this->openqrm->get('basedir').'/plugins/aoe-storage/web/storage/'.$resource->id.'.aoe.stat';
