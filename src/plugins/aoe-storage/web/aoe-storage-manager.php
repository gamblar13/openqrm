			...
			switch (htmlobject_request('action')) {
				case 'select':
					if (isset($_REQUEST['identifier'])) {
						foreach($_REQUEST['identifier'] as $id) {
							...
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/aoe-storage/bin/openqrm-aoe-storage post_luns -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
				...
				case 'add':
					if (strlen($aoe_storage_id)) {
						...
						$storage_resource = new resource();
						$storage_resource->get_instance_by_id($storage->resource_id);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/aoe-storage/bin/openqrm-aoe-storage add -n $aoe_lun_name -m $aoe_lun_size -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$storage_resource_id = $storage_resource->id;
						$statfile="storage/".$storage_resource_id.".aoe.stat";
						if (file_exists($statfile)) {
					...
					if (strlen($aoe_storage_id)) {
						...
						if (isset($_REQUEST['identifier'])) {
							...
							foreach($_REQUEST['identifier'] as $aoe_lun_name) {
								$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/aoe-storage/bin/openqrm-aoe-storage remove -n $aoe_lun_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
								$storage_resource->send_command($storage_resource->ip, $resource_command);
				...
				case 'snap':
					if (strlen($aoe_storage_id)) {
						...
						$storage_resource = new resource();
						$storage_resource->get_instance_by_id($storage->resource_id);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/aoe-storage/bin/openqrm-aoe-storage snap -n $aoe_lun_name -s $aoe_lun_snap_name -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						// remove current stat file
						$storage_resource_id = $storage_resource->id;
						$statfile="storage/".$storage_resource_id.".aoe.stat";
						if (file_exists($statfile)) {
...
function aoe_storage_display($aoe_storage_id) {
	...
	$table = new htmlobject_table_builder('aoe_lun_name', '', '', '', 'luns');
	$arHead = array();
