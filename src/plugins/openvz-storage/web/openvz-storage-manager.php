...
// to gather one of the deployment types within openvz-storage
...
function redirect_deploy($strMsg, $lvm_storage_id, $lvm_volume_group, $lvm_lun_name) {
	global $thisfile;
	global $action;
	$url = 'openvz-storage-deploy.php?strMsg='.urlencode($strMsg).'&currenttab=tab0&redirect=yes&action='.$action.'&lvm_storage_id='.$lvm_storage_id.'&lvm_volume_group='.$lvm_volume_group.'&lvm_lun_name='.$lvm_lun_name;
	echo "<meta http-equiv=\"refresh\" content=\"0; URL=$url\">";
	exit;
}
			...
			switch (htmlobject_request('action')) {
				...
				case 'select':
					if (isset($_REQUEST['identifier'])) {
						foreach($_REQUEST['identifier'] as $id) {
							...
							// post vg status
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/openvz-storage/bin/openqrm-openvz-storage post_vg -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
				...
				case 'select-vg':
					if (isset($_REQUEST['identifier'])) {
						foreach($_REQUEST['identifier'] as $lvm_volume_group) {
							...
							$storage_resource->get_instance_by_id($storage->resource_id);
							// post lv status
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/openvz-storage/bin/openqrm-openvz-storage post_lv -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password -v $lvm_volume_group";
							// remove current stat file
							$storage_resource_id = $storage_resource->id;
				...
				case 'add':
					...
					if (!strcmp($storage_deployment->type, "lvm-iscsi-deployment")) {
						...
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/openvz-storage/bin/openqrm-openvz-storage add -n $lvm_lun_name -v $lvm_volume_group -m $lvm_lun_size -i $image_password -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					} else {
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/openvz-storage/bin/openqrm-openvz-storage add -n $lvm_lun_name -v $lvm_volume_group -m $lvm_lun_size -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					}
					// remove current stat file
					$storage_resource_id = $storage_resource->id;
					$statfile="storage/".$storage_resource_id.".".$lvm_volume_group.".lv.stat";
				...
				case 'remove':
					if (isset($_REQUEST['identifier'])) {
						...
						foreach($_REQUEST['identifier'] as $lvm_lun_name) {
							...
							$storage_deployment = new deployment();
							$storage_deployment->get_instance_by_id($storage->type);
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/openvz-storage/bin/openqrm-openvz-storage remove -n $lvm_lun_name -v $lvm_volume_group -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
							// remove current stat file
							$storage_resource_id = $storage_resource->id;
							$statfile="storage/".$storage_resource_id.".".$lvm_volume_group.".lv.stat";
							if (file_exists($statfile)) {
				...
				case 'clone':
					...
					if (strlen($lvm_lun_snap_name)) {
						...
						if (!strcmp($deployment->type, "lvm-iscsi-deployment")) {
							...
							// generate a password for the image
							$image_password = $image->generatePassword(12);
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/openvz-storage/bin/openqrm-openvz-storage $lvm_action -n $lvm_lun_name -v $lvm_volume_group -s $lvm_lun_snap_name -m $lvm_lun_snap_size -i $image_password -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						} else {
							$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/openvz-storage/bin/openqrm-openvz-storage $lvm_action -n $lvm_lun_name -v $lvm_volume_group -s $lvm_lun_snap_name -m $lvm_lun_snap_size -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
						}
						// remove current stat file
						$storage_resource_id = $storage_resource->id;
						$statfile="storage/".$storage_resource_id.".".$lvm_volume_group.".lv.stat";
				...
				case 'resize':
					...
					if (!strcmp($deployment->type, "lvm-iscsi-deployment")) {
						...
						// generate a password for the image
						$image_password = $image->generatePassword(12);
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/openvz-storage/bin/openqrm-openvz-storage resize -n $lvm_lun_name -v $lvm_volume_group -m $lvm_lun_resize -i $image_password -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					} else {
						$resource_command="$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/openvz-storage/bin/openqrm-openvz-storage resize -n $lvm_lun_name -v $lvm_volume_group -m $lvm_lun_resize -u $OPENQRM_ADMIN->name -p $OPENQRM_ADMIN->password";
					}
					// remove current stat file
					$storage_resource_id = $storage_resource->id;
					$statfile="storage/".$storage_resource_id.".".$lvm_volume_group.".lv.stat";
				...
				case 'deploy':
					...
					$redir_msg = "Deploying volume $lvm_lun_name on Volume Group $lvm_volume_group";
...
function lvm_select_storage() {
	...
	$t_deployment->get_instance_by_type("openvz-deployment");
   ...
   // set template
	...
	$t->debug = false;
	$t->setFile('tplfile', './tpl/' . 'openvz-storage-select.tpl.php');
	$t->setVar(array(
		'formaction' => $thisfile,
...
function lvm_storage_display($lvm_storage_id) {
	...
	$t = new Template_PHPLIB();
	$t->debug = false;
	$t->setFile('tplfile', './tpl/' . 'openvz-storage-vgs.tpl.php');
	$t->setVar(array(
		'formaction' => $thisfile,
		'storage_name' => $storage->name,
...
function lvm_storage_lv_display($lvm_storage_id, $lvm_volume_group) {
	...
	// lvm table
	$table = new htmlobject_table_builder('lvm_lun_name', '', '', '', 'luns');
	$arHead = array();
	...
	$arHead['lvm_lun_deploy'] = array();
	$arHead['lvm_lun_deploy']['title'] ='';
	$arHead['lvm_lun_deploy']['sortable'] = false;
	...
	if (file_exists($storage_export_list)) {
		...
		foreach ($storage_vg_content as $index => $lvm) {
			...
			// build the deploy input
			$lvm_lun_deploy = "<form action=\"$thisfile\" method=\"GET\">";
			$lvm_lun_deploy .= "<input type='hidden' name='lvm_storage_id' value=$lvm_storage_id>";
			$lvm_lun_deploy .= "<input type='hidden' name='lvm_volume_group' value=$lvm_volume_group>";
			$lvm_lun_deploy .= "<input type='hidden' name='lvm_lun_name' value=$lvm_lun_name>";
			$lvm_lun_deploy .= "<input type='submit' name='action' value='deploy'>";
			$lvm_lun_deploy .= "</form>";
			...
			$arBody[] = array(
				...
				'lvm_lun_deploy' => "<nobr>".$lvm_lun_deploy."</nobr>",
			);
			$lvm_lun_count++;
		}
	}
	...
	$t = new Template_PHPLIB();
	$t->debug = false;
	$t->setFile('tplfile', './tpl/' . 'openvz-storage-luns.tpl.php');
	$t->setVar(array(
		'formaction' => $thisfile,
		'storage_name' => $storage->name,
