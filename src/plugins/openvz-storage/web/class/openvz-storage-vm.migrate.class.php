...
class openvz_storage_vm_migrate
{
/**
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/openvz-storage-vm-migrate.tpl.php');
	...
	function migrate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$statfile=$this->openqrm->get('basedir').'/plugins/openvz-storage/web/openvz-stat/'.$this->vm.'.vm_migrated_successfully';
			if ($this->file->exists($statfile)) {
				$this->file->remove($statfile);
			}

			$m_command     = $this->openqrm->get('basedir').'/plugins/openvz-storage/bin/openqrm-openvz-storage-vm migrate';
			$m_command    .= ' -n '.$this->vm;
			$m_command    .= ' -i '.$dest_host_resource->ip;
