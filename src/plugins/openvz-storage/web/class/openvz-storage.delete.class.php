<?php
/**
 * openvz-Storage delete Templates
 *
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/openvz-storage-delete.tpl.php');
		...
		if( $templates !== '' ) {
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($templates as $key => $template_name) {
					...
					$command  = $this->openqrm->get('basedir').'/plugins/openvz-storage/bin/openqrm-openvz-storage remove_openvz_template';
					$command .= ' -t '.$template_name;
					$file = $this->openqrm->get('basedir').'/plugins/openvz-storage/web/storage/openvz-templates.stat';
					...
					$this->openvztemplate->remove_by_name($template_name);
