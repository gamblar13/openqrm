...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/openvz-storage-vm-clone.tpl.php');
	...
	function duplicate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$command     = $this->openqrm->get('basedir').'/plugins/openvz-storage/bin/openqrm-openvz-storage-vm clone';
			$command    .= ' -n '.$this->vm;
			$command    .= ' -y '.$name;
	...
	function get_response() {
		...
		$d['name']['validate']['regex']             = '/^[0-9]+$/i';
		$d['name']['validate']['errormsg']          = sprintf($this->lang['error_name'], '0-9');
