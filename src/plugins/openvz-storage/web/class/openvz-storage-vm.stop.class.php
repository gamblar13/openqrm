	...
	function stop() {
		...
		if( $vms !== '' ) {
			...
			foreach($vms as $key => $vm) {
				...
				$command  = $this->openqrm->get('basedir').'/plugins/openvz-storage/bin/openqrm-openvz-storage-vm stop -n '.$vm;
						...
						foreach ($enabled as $index => $name) {
							...
							if (file_exists($hook)) {
								if (file_exists($running)) {
									$event->log("console", $_SERVER['REQUEST_TIME'], 5, "openvz-storage-vm.console.class.php", 'Found plugin '.$name.' providing a remote console.', "", "", 0, 0, $resource->id);
									require_once($hook);
