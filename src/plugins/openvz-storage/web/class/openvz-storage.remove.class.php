<?php
/**
 * openvz-Storage Remove Volume(s)
 *
...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/openvz-storage-remove.tpl.php');
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($lvols as $key => $lvol) {
					...
					if (count($appliances_using_resource) > 0) {
						...
						$command  = $this->openqrm->get('basedir').'/plugins/openvz-storage/bin/openqrm-openvz-storage remove';
						$command .= ' -n '.$lvol;
						$command .= ' -v '.$this->volgroup;
						$command .= ' -u '.$name.' -p '.$pass;

						$file = $this->openqrm->get('basedir').'/plugins/openvz-storage/web/storage/'.$resource->id.'.'.$this->volgroup.'.lv.stat';
