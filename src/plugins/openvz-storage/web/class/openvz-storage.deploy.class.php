<?php
/**
 * openvz-Storage deploy Templates
 *
...
var $lang = array();
	...
	function deploy() {
		...
		if (( $template !== '' ) && ( $lvol !== '' ) && ( $volgroup !== '' ) && ( $storage_id !== '' )) {
			...
			$command  = $this->openqrm->get('basedir').'/plugins/openvz-storage/bin/openqrm-openvz-storage deploy_openvz_template';
			$command .= ' -n '.$lvol;
			$command .= ' -v '.$volgroup;
			$command .= ' -t '.$template;
