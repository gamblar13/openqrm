...
var $lang = array();
	...
	function __construct($openqrm, $response, $controller) {
		...
		require_once($this->openqrm->get('webdir').'/plugins/opsi/class/opsi-volume.class.php');
	}
		...
		if(!$form->get_errors() && $this->response->submit()) {
			if(!$form->get_errors()) {
				...
				if(isset($error)) {
					...
					// add volume
					$volume_fields = array();
					$volume_fields["opsi_volume_id"] = openqrm_db_get_free_id('opsi_volume_id', $opsi_volume->_db_table);
