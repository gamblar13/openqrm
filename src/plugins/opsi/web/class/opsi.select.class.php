...
var $actions_name = 'opsi-select';
	...
	function action() {
		$table = $this->select();
		$template = $this->__response->html->template("./tpl/opsi-select.tpl.php");
...
//		$this->__response->html->debug();
		...
		$table = $this->__response->html->tablebuilder( 'opsi-table', $this->__response->get_array($this->actions_name, 'select'));
		...
		$deployment->get_instance_by_name("opsi-deployment");
		...
		if(count($storage_array) > 0) {
			...
			foreach ($storage_array as $index => $storage_db) {
				...
				if (!strcmp($storage_resource->state, "active")) {
					$state_content="<a href='https://".$storage_resource->ip.":4447/configed/' target='_BLANK'><img width=24 height=24 src=".$resource_icon_default."><br><small>(open UI)</small></a>";
				}
