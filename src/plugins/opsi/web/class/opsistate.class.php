...
function get_instance($id, $resource_id) {
	...
	if ("$id" != "") {
		$opsistate_array = &$db->Execute("select * from ".$LOCAL_STORAGE_STATE_TABLE." where opsi_id=".$id);
	} else if ("$resource_id" != "") {
		$opsistate_array = &$db->Execute("select * from ".$LOCAL_STORAGE_STATE_TABLE." where opsi_resource_id=".$resource_id);
	} else {
		$event->log("get_instance", $_SERVER['REQUEST_TIME'], 2, "opsistate.class.php", "Could not create instance of event without data", "", "", 0, 0, 0);
		return;
	}
...
function is_id_free($opsistate_id) {
	...
	global $event;
	$db=openqrm_get_db_connection();
	$rs = &$db->Execute("select opsi_id from ".$LOCAL_STORAGE_STATE_TABLE." where opsi_id=".$opsistate_id);
	if (!$rs)
		$event->log("is_id_free", $_SERVER['REQUEST_TIME'], 2, "opsistate.class.php", $db->ErrorMsg(), "", "", 0, 0, 0);
	else
	if ($rs->EOF) {
		return true;
	} else {
...
function add($opsistate_fields) {
	...
	if (!is_array($opsistate_fields)) {
		$event->log("add", $_SERVER['REQUEST_TIME'], 2, "opsistate.class.php", "opsistate_field not well defined", "", "", 0, 0, 0);
		return 1;
	}
	// set stop time and status to now
	$now=$_SERVER['REQUEST_TIME'];
	$db=openqrm_get_db_connection();
	$result = $db->AutoExecute($LOCAL_STORAGE_STATE_TABLE, $opsistate_fields, 'INSERT');
	if (! $result) {
		$event->log("add", $_SERVER['REQUEST_TIME'], 2, "opsistate.class.php", "Failed adding new opsistate to database", "", "", 0, 0, 0);
	}
}
...
function remove($opsistate_id) {
	global $LOCAL_STORAGE_STATE_TABLE;
	$db=openqrm_get_db_connection();
	$rs = $db->Execute("delete from ".$LOCAL_STORAGE_STATE_TABLE." where opsi_id=".$opsistate_id);
}
...
function remove_by_resource_id($opsistate_resource_id) {
	global $LOCAL_STORAGE_STATE_TABLE;
	$db=openqrm_get_db_connection();
	$rs = $db->Execute("delete from ".$LOCAL_STORAGE_STATE_TABLE." where opsi_resource_id=".$opsistate_resource_id);
}
...
function get_count() {
	...
	$count=0;
	$db=openqrm_get_db_connection();
	$rs = $db->Execute("select count(opsi_id) as num from ".$LOCAL_STORAGE_STATE_TABLE);
	if (!$rs) {
		print $db->ErrorMsg();
	} else {
		$count = $rs->fields["num"];
...
function get_all_ids() {
	...
	$query = "select opsi_id from ".$LOCAL_STORAGE_STATE_TABLE;
	$db=openqrm_get_db_connection();
	$rs = $db->Execute($query);
	if (!$rs)
		$event->log("get_list", $_SERVER['REQUEST_TIME'], 2, "opsistate.class.php", $db->ErrorMsg(), "", "", 0, 0, 0);
	else
	while (!$rs->EOF) {
