...
$strMsg = '';
	...
	switch (htmlobject_request('action')) {
		case 'map':
			$openqrm->send_command("$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/idoit/bin/openqrm-idoit-manager map");
			$strMsg .= "Now scanning the openQRM network to automatically add/update discovered systems in the CMDB. This will take some time ....";
			redirect($strMsg);
			break;
		case 'enable_automap':
			$openqrm->send_command("$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/idoit/bin/openqrm-idoit-manager automap -t on");
			$strMsg .= "Enabled automatic mapping of the openQRM Network";
			sleep(4);
			redirect($strMsg);
			break;
		case 'disable_automap':
			$openqrm->send_command("$OPENQRM_SERVER_BASE_DIR/openqrm/plugins/idoit/bin/openqrm-idoit-manager automap -t off");
...
function idoit_display() {
	...
	$t->debug = false;
	$t->setFile('tplfile', './tpl/' . 'idoit-overview-tpl.php');
	$t->setVar(array(
		'thisfile' => $thisfile,
		'automap' => $automap,
	));
...
$output = array();
$output[] = array('label' => 'I-doit Configuration', 'value' => idoit_display());

?>
<link rel="stylesheet" type="text/css" href="../../css/htmlobject.css" />
<link rel="stylesheet" type="text/css" href="idoit.css" />
<style>
.htmlobject_tab_box {
	width:400px;
}
