...
require_once "$RootDir/plugins/fai/class/fai.class.php";
...
var $actions_name = 'fai-select';
	...
	function action() {
		$table = $this->select();
		$template = $this->__response->html->template("./tpl/fai-select.tpl.php");
...
//		$this->__response->html->debug();
		...
		$table = $this->__response->html->tablebuilder( 'fai-table', $this->__response->get_array($this->actions_name, 'select'));
		...
		$deployment->get_instance_by_name("fai-deployment");
		...
		if(count($storage_array) > 0) {
			...
			foreach ($storage_array as $index => $storage_db) {
				...
				$storage_icon="/openqrm/base/plugins/fai/img/storage.png";
