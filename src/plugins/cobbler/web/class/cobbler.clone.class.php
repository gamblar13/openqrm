...
var $lang = array();
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/cobbler-clone.tpl.php');
	...
	function duplicate() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			$cobbler_volume->get_instance_by_name($name);
			if (isset($cobbler_volume->id) && $cobbler_volume->id > 0) {
			...
			if(isset($error)) {
				...
				// add volume
				$cobbler_volume->get_instance_by_name($this->volume);
				$volume_fields = array();
				$volume_fields["cobbler_volume_id"] = openqrm_db_get_free_id('cobbler_volume_id', $cobbler_volume->_db_table);
				$volume_fields['cobbler_volume_name'] = $name;
				$volume_fields['cobbler_volume_root'] = $cobbler_volume->root;
				$volume_fields['cobbler_volume_description'] = $cobbler_volume->description;
				$cobbler_volume->add($volume_fields);
				// add image
