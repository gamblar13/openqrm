...
var $lang = array(
	'select' => array (
		'tab' => 'Select Cobbler Server',
		'label' => 'Select Cobbler Server',
		'action_edit' => 'edit',
		'table_name' => 'Name',
		'table_id' => 'Id',
	...
	function __construct($openqrm, $response) {
		...
		$this->lang     = $this->user->translate($this->lang, $this->rootdir."/plugins/cobbler/lang", 'cobbler.ini');
	...
	//--------------------------------------------
	function api() {
		require_once($this->rootdir.'/plugins/cobbler/class/cobbler.api.class.php');
		$controller = new cobbler_api($this);
		$controller->action();
	}
	...
	function select( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/cobbler/class/cobbler.select.class.php');
			$controller = new cobbler_select($this->openqrm, $this->response);
	...
	function edit( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
		    require_once($this->rootdir.'/plugins/cobbler/class/cobbler.edit.class.php');
		    $controller                  = new cobbler_edit($this->openqrm, $this->response);
		    $controller->actions_name    = $this->actions_name;
		    $controller->tpldir          = $this->tpldir;
		    $controller->message_param   = $this->message_param;
	...
	function add( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
		    require_once($this->rootdir.'/plugins/cobbler/class/cobbler.add.class.php');
		    $controller                = new cobbler_add($this->openqrm, $this->response, $this);
		    $controller->actions_name  = $this->actions_name;
		    $controller->tpldir        = $this->tpldir;
		    $controller->message_param = $this->message_param;
	...
	function remove( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
		    require_once($this->rootdir.'/plugins/cobbler/class/cobbler.remove.class.php');
		    $controller                  = new cobbler_remove($this->openqrm, $this->response);
		    $controller->actions_name    = $this->actions_name;
		    $controller->tpldir          = $this->tpldir;
		    $controller->message_param   = $this->message_param;
		    $controller->identifier_name = $this->identifier_name;
	...
	function duplicate( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
		    require_once($this->rootdir.'/plugins/cobbler/class/cobbler.clone.class.php');
		    $controller                  = new cobbler_clone($this->openqrm, $this->response);
		    $controller->actions_name    = $this->actions_name;
		    $controller->tpldir          = $this->tpldir;
		    $controller->message_param   = $this->message_param;
		    $controller->identifier_name = $this->identifier_name;
