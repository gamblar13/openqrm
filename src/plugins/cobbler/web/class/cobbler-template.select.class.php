...
var $actions_name = 'cobbler-select';
	...
	function action() {
		$table = $this->select();
		$template = $this->response->html->template($this->tpldir."/cobbler-template-select.tpl.php");
...
//		$this->response->html->debug();
		...
		$table = $this->response->html->tablebuilder( 'cobbler-table', $this->response->get_array($this->actions_name, 'select'));
		...
		if(count($storage_array) > 0) {
			...
			foreach ($storage_array as $index => $storage_db) {
				...
				if (!strcmp($storage_resource->state, "active")) {
					$state_content="<a href='http://".$storage_resource->ip."/cobbler_web/' target='_BLANK'><img width=24 height=24 src=".$resource_icon_default."><br><small>(open UI)</small></a>";
				}
			...
			$table->body = $arBody;
			$table->max = $storage_tmp->get_count_per_type($storage->type);
