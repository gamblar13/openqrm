...
var $lang = array();
	...
	function init() {
		...
		$this->statfile_vm_config = $this->rootdir.'/plugins/citrix/citrix-stat/'.$resource->ip.'.'.$vm_name.'.vm_config';
	...
	function action() {
		...
		$t = $this->response->html->template($this->tpldir.'/citrix-vm-update.tpl.php');
	...
	function vm_update() {
		...
		if(!$form->get_errors() && $this->response->submit()) {
			...
			if(isset($error)) {
				...
				$command = $this->openqrm->get('basedir')."/plugins/citrix/bin/openqrm-citrix-vm update -i ".$this->resource->ip." -n ".$name." -l ".$datastore." -m ".$mac." -c ".$cpu." -r ".$memory." ".$additional_nic_parameter." -b ".$bootorder;
	...
	function get_response() {
		...
		if (file_exists($this->statfile_vm_config)) {
			...
			if (isset($ini['OPENQRM_CITRIX_VM_MAC_1'])) {
				$additional_nic_mac[1] = $ini['OPENQRM_CITRIX_VM_MAC_1'];
				$add_nic_loop++;
			} else {
			...
			if (isset($ini['OPENQRM_CITRIX_VM_MAC_2'])) {
				$additional_nic_mac[2] = $ini['OPENQRM_CITRIX_VM_MAC_2'];
				$add_nic_loop++;
			} else {
				$mac_gen = new resource();
				$mac_gen->generate_mac();
			...
			if (isset($ini['OPENQRM_CITRIX_VM_MAC_3'])) {
				$additional_nic_mac[3] = $ini['OPENQRM_CITRIX_VM_MAC_3'];
				$add_nic_loop++;
			} else {
				$mac_gen = new resource();
				$mac_gen->generate_mac();
			...
			if (isset($ini['OPENQRM_CITRIX_VM_MAC_4'])) {
				$additional_nic_mac[4] = $ini['OPENQRM_CITRIX_VM_MAC_4'];
				$add_nic_loop++;
			} else {
				$mac_gen = new resource();
				$mac_gen->generate_mac();
		...
		if (file_exists($this->statfile_template)) {
			...
			if(count($lines) >= 1) {
				foreach($lines as $line) {
					if($line !== '') {
						$line = explode(':', $line);
						if ($line[0] != $ini['OPENQRM_CITRIX_VM_TEMPLATE']) {
							continue;
						}
		...
		$d['memory']['object']['attrib']['selected']			= array($ini['OPENQRM_CITRIX_VM_RAM']);
		...
		$d['cpu']['object']['attrib']['selected']				= array($ini['OPENQRM_CITRIX_VM_CPUS']);
		...
		$d['datastore_disabled']['object']['attrib']['value']		    = $ini['OPENQRM_CITRIX_VM_SR_NAME'];
		...
		$d['datastore']['object']['attrib']['value']			= $ini['OPENQRM_CITRIX_VM_SR_UUID'];
...
//		$d['mac']['validate']['regex']							= '/^[a-z0-9._-]+$/i';
		...
		$d['mac']['object']['attrib']['value']					= $ini['OPENQRM_CITRIX_VM_MAC_0'];
		...
		$d['vm_mac']['object']['attrib']['value']				= $ini['OPENQRM_CITRIX_VM_MAC_0'];
...
//		$boot_order_select_arr [] = array("value" => 'cd', "label" => 'cd');
		...
		$d['boot_order']['object']['attrib']['selected']= array($ini['OPENQRM_CITRIX_VM_BOOT']);
