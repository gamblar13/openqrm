...
var $lang = array(
	'select' => array (
		...
		'table_type' => 'Type',
		'error_no' => '<b>No storage configured yet!</b><br><br>Please create a Citrix XenServer Storage first!',
		'new' => 'New Storage',
	...
	function __construct($openqrm, $response) {
		...
		$this->lang = $this->user->translate($this->lang, $this->rootdir."/plugins/citrix/lang", 'citrix.ini');
	...
	function select( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/citrix/class/citrix.select.class.php');
			$controller = new citrix_select($this->openqrm, $this->response);
...
// #############################################################################
	...
	function vm( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_vm()) {
				require_once($this->rootdir.'/plugins/citrix/class/citrix.vm.class.php');
				$controller                  = new citrix_vm($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
	...
	function reload_vm() {
		...
		$command  = $this->basedir."/plugins/citrix/bin/openqrm-citrix-vm post_vm_list -i ".$resource->ip;
		$openqrm = new resource();
		$openqrm->get_instance_by_id(0);
		$file = $this->rootdir.'/plugins/citrix/citrix-stat/'.$resource->ip.'.vm_list';
		if(file_exists($file)) {
			unlink($file);
		}
	...
	function reload_vm_templates() {
		...
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$command  = $this->basedir."/plugins/citrix/bin/openqrm-citrix-vm post_template_list -i ".$resource->ip;
		$openqrm = new resource();
		$openqrm->get_instance_by_id(0);
		$file = $this->rootdir.'/plugins/citrix/citrix-stat/'.$resource->ip.'.template_list';
		if(file_exists($file)) {
			unlink($file);
		}
		$resource->send_command($openqrm->ip, $command);
	...
	function reload_vm_config() {
		...
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$command  = $this->basedir."/plugins/citrix/bin/openqrm-citrix-vm post_vm_config -i ".$resource->ip." -n ".$vm_name;
		$openqrm = new resource();
		$openqrm->get_instance_by_id(0);
		$file = $this->rootdir.'/plugins/citrix/citrix-stat/'.$resource->ip.'.'.$vm_name.'.vm_config';
		if(file_exists($file)) {
			unlink($file);
		}
		$resource->send_command($openqrm->ip, $command);
	...
	function vm_add( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if (($this->reload_ds()) && ($this->reload_vm_templates())) {
				require_once($this->rootdir.'/plugins/citrix/class/citrix.vm-add.class.php');
				$controller                  = new citrix_vm_add($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function vm_update( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if (($this->reload_ds()) && ($this->reload_vm_config()) && ($this->reload_vm_templates())) {
				require_once($this->rootdir.'/plugins/citrix/class/citrix.vm-update.class.php');
				$controller                  = new citrix_vm_update($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function vm_remove( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_vm()) {
				require_once($this->rootdir.'/plugins/citrix/class/citrix.vm-remove.class.php');
				$controller                  = new citrix_vm_remove($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function vm_start( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_vm()) {
				require_once($this->rootdir.'/plugins/citrix/class/citrix.vm-start.class.php');
				$controller                  = new citrix_vm_start($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function vm_stop( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_vm()) {
				require_once($this->rootdir.'/plugins/citrix/class/citrix.vm-stop.class.php');
				$controller                  = new citrix_vm_stop($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function vm_console( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/citrix/class/citrix.vm-console.class.php');
			$controller                  = new citrix_vm_console($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function vm_boot( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_vm()) {
				require_once($this->rootdir.'/plugins/citrix/class/citrix.vm-boot.class.php');
				$controller                  = new citrix_vm_boot($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function vm_clone( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ds()) {
				require_once($this->rootdir.'/plugins/citrix/class/citrix.vm-clone.class.php');
				$controller                  = new citrix_vm_clone($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
...
// #############################################################################
	...
	function ds( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ds()) {
				require_once($this->rootdir.'/plugins/citrix/class/citrix.ds.class.php');
				$controller                  = new citrix_ds($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function reload_ds() {
		...
		$resource = new resource();
		$resource->get_instance_by_id($appliance->resources);
		$command  = $this->basedir."/plugins/citrix/bin/openqrm-citrix-datastore post_ds_list -i ".$resource->ip;
		$openqrm = new resource();
		$openqrm->get_instance_by_id(0);
		$file = $this->rootdir.'/plugins/citrix/citrix-stat/'.$resource->ip.'.ds_list';
		if(file_exists($file)) {
			unlink($file);
		}
		$resource->send_command($openqrm->ip, $command);
	...
	function ds_add_nas( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ds()) {
				require_once($this->rootdir.'/plugins/citrix/class/citrix.ds-add-nas.class.php');
				$controller                  = new citrix_ds_add_nas($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ds_remove_nas( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ds()) {
				require_once($this->rootdir.'/plugins/citrix/class/citrix.ds-remove-nas.class.php');
				$controller                  = new citrix_ds_remove_nas($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ds_add_iscsi( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ds()) {
				require_once($this->rootdir.'/plugins/citrix/class/citrix.ds-add-iscsi.class.php');
				$controller                  = new citrix_ds_add_iscsi($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
	...
	function ds_remove_iscsi( $hidden = true ) {
		...
		if( $hidden === true ) {
		    if($this->reload_ds()) {
				require_once($this->rootdir.'/plugins/citrix/class/citrix.ds-remove-iscsi.class.php');
				$controller                  = new citrix_ds_remove_iscsi($this->openqrm, $this->response);
				$controller->actions_name    = $this->actions_name;
				$controller->tpldir          = $this->tpldir;
				$controller->message_param   = $this->message_param;
				$controller->identifier_name = $this->identifier_name;
...
// #############################################################################
	...
	function reboot( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/citrix/class/citrix.reboot.class.php');
			$controller                  = new citrix_reboot($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
	...
	function shutdown( $hidden = true ) {
		$data = '';
		if( $hidden === true ) {
			require_once($this->rootdir.'/plugins/citrix/class/citrix.shutdown.class.php');
			$controller                  = new citrix_shutdown($this->openqrm, $this->response);
			$controller->actions_name    = $this->actions_name;
			$controller->tpldir          = $this->tpldir;
			$controller->message_param   = $this->message_param;
			$controller->identifier_name = $this->identifier_name;
