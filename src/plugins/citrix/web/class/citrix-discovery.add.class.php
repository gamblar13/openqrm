...
var $lang = array();
	...
	//--------------------------------------------
	function action() {
		if($this->response->html->request()->get($this->identifier_name) !== '' || $this->response->html->request()->get('citrix_ad_id') !== '') {
			$response = $this->add();
			...
			$t = $response->html->template($this->tpldir.'/citrix-discovery-add.tpl.php');
	...
	function add() {
		...
		if(!$form->get_errors()	&& $response->submit()) {
			...
			$this->discovery->get_instance_by_id($data['citrix_ad_id']);
			if ($this->discovery->xenserver_ad_is_integrated > 0) {
				$response->error = sprintf($this->lang['error_exists'], $this->discovery->xenserver_ad_id);
				return $response;
			}
			...
			if(!$form->get_errors()) {
				...
				$command  = $this->openqrm->get('basedir')."/plugins/citrix/bin/openqrm-citrix configure -i ".$esx_ip." -eu ".$esx_user." -ep ".$esx_password." -eh ".$esx_hostname;
				$file = $this->rootdir."/plugins/citrix/citrix-stat/".$esx_ip.".integrated_successful";
				if(file_exists($file)) {
					unlink($file);
				}
						...
						foreach($lines as $line) {
							if($line !== '') {
								if (!strstr($line, "success")) {
									$response->error = sprintf($this->lang['error_integrating'], $this->discovery->xenserver_ad_id);
									return $response;
								}
							}
						}
				...
				$esx_virtualization->get_instance_by_type('citrix');
				...
				foreach ($enabled_plugins as $index => $plugin_name) {
					...
					if (file_exists($plugin_start_appliance_hook)) {
						$event->log("integrate", $_SERVER['REQUEST_TIME'], 5, "citrix-discovery.add.class.php", "Found plugin $plugin_name handling start-appliance event.", "", "", 0, 0, $appliance->resources);
				...
				$this->discovery->update($data['citrix_ad_id'], $discovery_esx_fields);
	...
	function get_response() {
		...
		if (strlen($this->discovery->xenserver_ad_user)) {
			$citrix_username = $this->discovery->xenserver_ad_user;
		} else {
			$citrix_username = "root";
		}
		...
		$d['citrix_ad_id']['label']                     = "Discovery ID";
		$d['citrix_ad_id']['required']                  = true;
		$d['citrix_ad_id']['validate']['regex']         = '~^[0-9]+$~i';
		$d['citrix_ad_id']['validate']['errormsg']      = 'ID must be [0-9] only';
		$d['citrix_ad_id']['object']['type']            = 'htmlobject_input';
		$d['citrix_ad_id']['object']['attrib']['type']  = 'text';
		$d['citrix_ad_id']['object']['attrib']['id']    = 'citrix_ad_id';
		$d['citrix_ad_id']['object']['attrib']['name']  = 'citrix_ad_id';
		$d['citrix_ad_id']['object']['attrib']['value']  = $this->discovery->xenserver_ad_id;
...
//		$d['xenserver_ad_hostname']['validate']['errormsg']      = 'Hostname must be [a-z0-9] only';
		...
		$d['xenserver_ad_user']['object']['attrib']['value']  = $citrix_username;
