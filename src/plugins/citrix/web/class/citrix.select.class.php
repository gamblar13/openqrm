...
var $lang = array();
	...
	function action() {
		$table = $this->select();
		$t = $this->response->html->template($this->tpldir.'/citrix-select.tpl.php');
	...
	function select() {
		...
		$citrix_array = $appliance->display_overview_per_virtualization($virtualization->id, $table->offset, $table->limit, $table->sort, $table->order);
		$ta = '';
		foreach ($citrix_array as $index => $citrix) {
