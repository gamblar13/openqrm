...
var $actions_name = 'cloud_selector';
	...
	function action() {
		...
		foreach ($virtualization_list as $virtualization_arr) {
			$o = $html->option();
			$o->value = $virtualization_arr['value'];
			$o->label = $virtualization_arr['label'];
			$s->add($o);
		}

		$template = $this->response->html->template($this->tpldir."/cloud-selector-resource.tpl.php");
		...
		$template->add($s, 'resource_select');
	...
	function resource() {
		...
		$form = $response->get_form($this->actions_name, 'resource');
		$response->form = $form;
		...
		$table = $response->html->tablebuilder( 'cloud-selector-table', $this->response->get_array($this->actions_name, 'resource'));
		...
		$table->sort            = 'sort_id';
		$table->autosort        = true;
		$table->max				= $this->cloudselector->get_count_by_type("resource");
		$table->identifier      = 'id';
		...
		$cloud_selector_array = $this->cloudselector->display_overview_per_type("resource");
		$ta = '';
