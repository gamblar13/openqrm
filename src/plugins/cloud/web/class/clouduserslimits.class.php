	...
	function check_limits($resource_quantity, $ram_req, $disk_req, $cpu_req, $network_req) {
		...
		if ($this->resource_limit != 0) {
			...
			if ($users_appliance_count >= $this->resource_limit) {
				$this->_event->log("check_limits", $_SERVER['REQUEST_TIME'], 2, "clouduserlimits.class.php", "-> Not adding request from User $cu_id who has already $users_appliance_count appliance(s) running.", "", "", 0, 0, 0);
				return false;
			} else {
