...
var $actions_name = 'cloud_selector';
	...
	function action() {
		...
		$puppet_list = array();
		$show_puppet_groups = $this->cloud_config->get_value_by_key('show_puppet_groups');
		if (!strcmp($show_puppet_groups, "true")) {
			// is puppet enabled ?
			if (file_exists($this->webdir."/plugins/puppet/.running")) {
				require_once $this->webdir."/plugins/puppet/class/puppet.class.php";
				$puppet_group_dir = $this->webdir."/plugins/puppet/puppet/manifests/groups";
		...
		$template = $this->response->html->template($this->tpldir."/cloud-selector-puppet.tpl.php");
	...
	function puppet() {
		...
		$form = $response->get_form($this->actions_name, 'puppet');
		$response->form = $form;
		...
		$table = $response->html->tablebuilder( 'cloud-selector-table', $this->response->get_array($this->actions_name, 'puppet'));
		...
		$table->sort            = 'sort_id';
		$table->autosort        = true;
		$table->max				= $this->cloudselector->get_count_by_type("puppet");
		$table->identifier      = 'id';
		...
		$cloud_selector_array = $this->cloudselector->display_overview_per_type("puppet");
		$ta = '';
