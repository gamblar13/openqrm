...
var $tpldir;
var $lang;
var $actions_name = 'cloud-resource-pool-update';
		...
		if(!$form->get_errors()	&& $response->submit()) {
			...
			if(!$form->get_errors()) {
				...
				if ($this->cloudrespool->exists_by_resource_id($this->appliance->resources)) {
					...
					// new
					$private_cloud_resource_pool_fields["rp_id"] = openqrm_db_get_free_id('rp_id', $this->cloudrespool->_db_table);
	...
	function get_response($mode) {
		...
		$cloud_resource_pool_assign_default_arr[] = array( 'value' => '0', 'label' => $this->lang['cloud_resource_pool_everybody']);
