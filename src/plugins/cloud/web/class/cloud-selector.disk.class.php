...
var $actions_name = 'cloud_selector';
	...
	function action() {
		$response = $this->disk();
		$template = $this->response->html->template($this->tpldir."/cloud-selector-disk.tpl.php");
	...
	function disk() {
		...
		$form = $response->get_form($this->actions_name, 'disk');
		$response->form = $form;
		...
		$table = $response->html->tablebuilder( 'cloud-selector-table', $this->response->get_array($this->actions_name, 'disk'));
		...
		$table->sort            = 'sort_id';
		$table->autosort        = true;
		$table->max				= $this->cloudselector->get_count_by_type("disk");
		$table->identifier      = 'id';
		...
		$cloud_selector_array = $this->cloudselector->display_overview_per_type("disk");
		$ta = '';
