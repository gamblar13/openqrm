...
//		$$ha_select_input = htmlobject_select('product_quantity', $ha_list_select, '');
		...
		$template = $this->response->html->template($this->tpldir."/cloud-selector-ha.tpl.php");
	...
	function ha() {
		...
		$form = $response->get_form($this->actions_name, 'ha');
		$response->form = $form;
		...
		$table = $response->html->tablebuilder( 'cloud-selector-table', $this->response->get_array($this->actions_name, 'ha'));
		...
		$table->sort            = 'sort_id';
		$table->autosort        = true;
		$table->max				= $this->cloudselector->get_count_by_type("ha");
		$table->identifier      = 'id';
		...
		$cloud_selector_array = $this->cloudselector->display_overview_per_type("ha");
		$ta = '';
