...
var $actions_name = 'cloud_selector';
	...
	function action() {
		...
		foreach ($kernel_list as $kernel_arr) {
			$o = $html->option();
			$o->value = $kernel_arr['value'];
			$o->label = $kernel_arr['label'];
			$s->add($o);
		}

		$template = $this->response->html->template($this->tpldir."/cloud-selector-kernel.tpl.php");
	...
	function kernel() {
		...
		$form = $response->get_form($this->actions_name, 'kernel');
		$response->form = $form;
		...
		$table = $response->html->tablebuilder( 'cloud-selector-table', $this->response->get_array($this->actions_name, 'kernel'));
		...
		$table->sort            = 'sort_id';
		$table->autosort        = true;
		$table->max				= $this->cloudselector->get_count_by_type("kernel");
		$table->identifier      = 'id';
		...
		$cloud_selector_array = $this->cloudselector->display_overview_per_type("kernel");
		$ta = '';
