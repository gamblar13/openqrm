...
var $actions_name = 'cloud_selector';
	...
	function action() {
		$response = $this->cpu();
		$template = $this->response->html->template($this->tpldir."/cloud-selector-cpu.tpl.php");
	...
	function cpu() {
		...
		$form = $response->get_form($this->actions_name, 'cpu');
		$response->form = $form;
		...
		$table = $response->html->tablebuilder( 'cloud-selector-table', $this->response->get_array($this->actions_name, 'cpu'));
		...
		$table->sort            = 'sort_id';
		$table->autosort        = true;
		$table->max				= $this->cloudselector->get_count_by_type("cpu");
		$table->identifier      = 'id';
		...
		$cloud_selector_array = $this->cloudselector->display_overview_per_type("cpu");
		$ta = '';
