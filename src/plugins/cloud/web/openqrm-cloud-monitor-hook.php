...
function openqrm_cloud_monitor() {
	...
	foreach($cr_list as $list) {
		...
		$arr = array('@@ID@@'=>"$cr_id", '@@FORENAME@@'=>"$cu_forename", '@@LASTNAME@@'=>"$cu_lastname", '@@START@@'=>"$start", '@@STOP@@'=>"$stop", '@@PASSWORD@@'=>"$appliance_password", '@@IP@@'=>"$resource_external_ip", '@@RESNUMBER@@'=>"$cr_resource_number", '@@HOSTNAME@@'=> $appliance->name);
		$rmail->var_array = $arr;
		$rmail->send();
		...
		if ($cloud_billing_enabled != 'true') {
			...
			if ($cu_ccunits <= $cloud_deprovision_warning) {
				...
				if (($currenttimestamp - $lastwarning >= $one_day) || ($check_cr_now == 'true')) {
					...
					$arr = array('@@ID@@'=>"$cr_id", '@@FORENAME@@'=>"$cu->forename", '@@LASTNAME@@'=>"$cu->lastname", '@@CCUS@@'=>"$cu_ccunits");
			...
			if ($cu_ccunits <= 0) {
				...
				$arr = array('@@ID@@'=>"$cr_id", '@@FORENAME@@'=>"$cu->forename", '@@LASTNAME@@'=>"$cu->lastname");
