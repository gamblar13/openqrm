...
//		}
		...
		$template->add($this->lang['cloud_ui_confirm_pause'], 'confirm_pause');
			...
			if(isset($request) && is_array($request)) {
				...
				foreach($request as $key => $cloudappliance_id) {
					...
					if ($this->cloudappliance->state == 1) {
						...
						$this->cloudmailer->subject = "openQRM Cloud: Cloud Appliance $cloudappliance_id registered for stop (pause)";
						...
						$message[] = $this->lang['cloud_ui_paused']." ".$cloudappliance_id;
					} else {
						$errors[] = $this->lang['cloud_ui_pause_failed']." ".$cloudappliance_id;
						continue;
					}
				}
				if(count($errors) === 0) {
