...
if (htmlobject_request('action') != '') {
	switch (htmlobject_request('action')) {
		case 'newvcd':
			...
			if (isset($_GET["systemtype"])) {
				$systemtype=$_GET["systemtype"];
			} else {
				$systemtype = '';
			}
			if (isset($_GET["serverimage"])) {
			...
			if (isset($_GET["application3"])) {
				$application3 =$_GET["application3"];
			} else {
				$application3 = '';
			}
			if (isset($_GET["application4"])) {
				$application4 =$_GET["application4"];
			} else {
				$application4 = '';
			}
			if (isset($_GET["cr_start"])) {
				$cr_start = $_GET['cr_start'];
			} else {
				$cr_start = '';
			}
			if (isset($_GET["cr_stop"])) {
