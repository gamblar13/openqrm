...
function get_count_active() {
	...
	$count=0;
	$db=openqrm_get_db_connection();
	$rs = $db->Execute("select count(appliance_id) as num from ".$APPLIANCE_INFO_TABLE." where appliance_state='active'");
	if (!$rs) {
		print $db->ErrorMsg();
	} else {
		$count = $rs->fields["num"];
...
function get_ids_per_resource($resource_id) {
	...
	$rs = $db->Execute($query);
	if (!$rs)
		$event->log("get_ids_per_resource", $_SERVER['REQUEST_TIME'], 2, "appliance.class.php", $db->ErrorMsg(), "", "", 0, 0, 0);
	else
	while (!$rs->EOF) {
		$appliance_list[] = $rs->fields;
		$rs->MoveNext();
...
function get_ids_per_image($image_id) {
	...
	$rs = $db->Execute($query);
	if (!$rs) {
		$event->log("get_ids_per_image", $_SERVER['REQUEST_TIME'], 2, "appliance.class.php", $db->ErrorMsg(), "", "", 0, 0, 0);
	} else {
		while (!$rs->EOF) {
			$appliance_list[] = $rs->fields;
			$rs->MoveNext();
