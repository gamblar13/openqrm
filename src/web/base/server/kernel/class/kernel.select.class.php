		...
		foreach ($kernel_arr as $index => $kernel_db) {
			...
			if ($kernel_db["kernel_id"] != 1) {
				if ((strncmp($kernel->name, "resource", 7)) && (!strstr($kernel->capabilities, "local-server"))){
					...
					$a->css     = 'default';
					$a->href    = $this->response->get_url($this->actions_name, 'setdefault').'&'.$this->identifier_name.'[]='.$kernel->id;
			...
			$a->handler = 'onclick="wait();"';
			$a->css     = 'edit';
			$a->href    = $this->response->get_url($this->actions_name, 'edit').'&'.$this->identifier_name.'='.$kernel->id;
			$edit = $a->get_string();
