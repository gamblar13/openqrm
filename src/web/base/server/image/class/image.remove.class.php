...
var $lang = array();
	...
	function remove() {
		...
		$images = $response->html->request()->get($this->identifier_name);
		$form     = $response->form;
		$image = new image();
		if( $images !== '' ) {
			...
			foreach($images as $id) {
				...
				$image = $image->get_instance_by_id($id);

				$d['param_f'.$i]['label']                       = $image->name;
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($images as $id) {
					...
					if ($remove_error == 1) {
						$message[] = sprintf($this->lang['msg_not_removing_active'], $id, $image_is_used_by_appliance);
						continue;
					}
